...
- Reports that require write access to trusted local state (`~/.dex`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Anyone who can modify `~/.dex` state/config (including `openclaw.json`) is effectively a trusted operator.
