...
- The gateway host also needs direct APNs auth configured separately with `DEX_APNS_TEAM_ID`, `DEX_APNS_KEY_ID`, and either `DEX_APNS_PRIVATE_KEY_P8` or `DEX_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.dex/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `DEX_APNS_PRIVATE_KEY_PATH` at that file.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `openclaw.json`. `DEX_APNS_RELAY_BASE_URL` remains a temporary env override only.
