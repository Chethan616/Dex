...
- Inbound, outbound, and gateway bridge paths share a single media payload root under `~/.dex/media`, so uploads, downloads, and transcode caches land under one guarded directory instead of a per-subsystem tree.
