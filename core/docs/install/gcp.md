...
- Persist `~/.dex` + `~/.dex/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.dex` state includes `openclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    DEX_IMAGE=openclaw:latest
    DEX_GATEWAY_TOKEN=
    DEX_GATEWAY_BIND=lan
    DEX_GATEWAY_PORT=18789

    DEX_CONFIG_DIR=/home/$USER/.openclaw
    DEX_WORKSPACE_DIR=/home/$USER/.openclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.openclaw
    ```

    Set `DEX_GATEWAY_TOKEN` when you want to manage the stable gateway
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `DEX_GATEWAY_TOKEN`.
    ...
    services:
      openclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - DEX_GATEWAY_BIND=${DEX_GATEWAY_BIND}
          - DEX_GATEWAY_PORT=${DEX_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${DEX_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`DEX_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:
