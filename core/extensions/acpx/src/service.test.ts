...
import type { DexPluginServiceContext } from "../runtime-api.js";
import { createAcpxRuntimeService, resolveAcpxTimerTimeoutMs } from "./service.js";

const tempDirs: string[] = [];
const previousEnv = {
  DEX_ACPX_RUNTIME_STARTUP_PROBE: process.env.DEX_ACPX_RUNTIME_STARTUP_PROBE,
  DEX_SKIP_ACPX_RUNTIME: process.env.DEX_SKIP_ACPX_RUNTIME,
  DEX_SKIP_ACPX_RUNTIME_PROBE: process.env.DEX_SKIP_ACPX_RUNTIME_PROBE,
};
...
afterEach(async () => {
  ...
  restoreEnv("DEX_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("DEX_SKIP_ACPX_RUNTIME");
  restoreEnv("DEX_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): DexPluginServiceContext {
  return {
    workspaceDir,
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.DEX_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.DEX_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.DEX_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.DEX_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.DEX_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.DEX_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.DEX_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (DEX_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
