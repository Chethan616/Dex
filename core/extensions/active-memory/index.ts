...
import { definePluginEntry, type DexPluginApi } from "openclaw/plugin-sdk/plugin-entry";
...
function resolveDefaultToolsAllow(cfg: DexConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: DexConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: DexPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: DexPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: DexConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: DexPluginApi): DexConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as DexConfig | undefined) ??
      (api.config as DexConfig | undefined)
    );
  } catch {
    return api.config as DexConfig | undefined;
  }
}
...
export default definePluginEntry({
  ...
  register(api: DexPluginApi) {
    const readCurrentConfig = (): DexConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as DexConfig | undefined) ??
          (api.config as DexConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as DexConfig;
          if (action === "status") {
            return {
