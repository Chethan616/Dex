...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.DEX_GATEWAY_PORT = prevGatewayPort;
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.DEX_GATEWAY_PORT;
  process.env.DEX_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.DEX_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.DEX_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.DEX_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.DEX_GATEWAY_PASSWORD;
  } else {
    process.env.DEX_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
