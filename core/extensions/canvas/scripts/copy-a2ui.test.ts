...
const ORIGINAL_SKIP_MISSING = process.env.DEX_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.DEX_SPARSE_PROFILE;

describe("canvas a2ui copy", () => {
  beforeEach(() => {
    delete process.env.DEX_A2UI_SKIP_MISSING;
    delete process.env.DEX_SPARSE_PROFILE;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      ...
      process.env.DEX_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.DEX_SPARSE_PROFILE;
    } else {
      process.env.DEX_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });
  ...
  it("skips missing assets when DEX_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.DEX_A2UI_SKIP_MISSING = "1";
      await expect(
  ...
  it("skips missing assets when DEX_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.DEX_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: dir, outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
