...
import type { DexPluginNodeInvokePolicyContext } from "openclaw/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: DexPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<DexPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<DexPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<DexPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
