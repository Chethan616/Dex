...
type WebSearchConfig = NonNullable<DexConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<DexConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DexConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: DexConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: DexConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: DexConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: DexConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: DexConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: DexConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: DexConfig, override?: number): number {
