...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "DEX_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "DEX_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "DEX_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["DEX_LIVE_ANTHROPIC_KEY"],
  gemini: ["DEX_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "DEX_LIVE_CODEX_API_KEY",
    "DEX_LIVE_OPENAI_KEY",
    "OPENAI_API_KEY",
  ],
});
...
function qaLiveOpenAiUsesCodexByDefault(cfg: DexConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: DexConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: DexConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.DEX_QA_FORCE_RUNTIME?.trim().toLowerCase();
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or DEX_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
