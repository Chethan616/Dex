...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the OpenClaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("DEX_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("DEX_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses DEX_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("DEX_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("preserves caller-provided custom fetch when DEX_PROXY_URL is present", async () => {
    vi.stubEnv("DEX_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over DEX_PROXY_URL for Telegram", async () => {
    vi.stubEnv("DEX_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
