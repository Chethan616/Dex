...
export function resolveCompileConcurrency(
  ...
  const raw = env.DEX_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "DEX_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.DEX_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
