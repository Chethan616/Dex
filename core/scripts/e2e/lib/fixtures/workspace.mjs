...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.DEX_WORKSPACE_DIR || path.join(process.env.HOME, ".dex", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.DEX_STATE_DIR, "DEX_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.DEX_GATEWAY_TOKEN?.trim();
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.DEX_STATE_DIR, "openclaw.json"))?.agents?.list ?? [];
