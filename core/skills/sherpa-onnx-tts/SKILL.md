...
1. Download the runtime for your OS (extracts into `$DEX_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.dex/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$DEX_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.dex/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active OpenClaw config file (`$DEX_CONFIG_PATH`, default `~/.dex/openclaw.json`):
