...
type MiddlewareContentBlock = DexAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is DexAgentToolResult {
...
function coerceMiddlewareToolResult(
  ...
  const content: DexAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: DexAgentToolResult): DexAgentToolResult {
...
function buildMiddlewareFailureResult(): DexAgentToolResult {
  return {
    content: [
      {
        type: "text",
