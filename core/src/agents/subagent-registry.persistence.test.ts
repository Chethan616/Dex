...
describe("subagent registry persistence", () => {
  ...
  it("reuses the persisted registry cache on hot internal read snapshots", async () => {
    ...
    const previousFlag = process.env.DEX_TEST_READ_SUBAGENT_RUNS_FROM_DISK;
    let cloneSpy: { mockRestore(): void } | undefined;
    try {
      process.env.DEX_TEST_READ_SUBAGENT_RUNS_FROM_DISK = "1";
      ...
      if (previousFlag === undefined) {
        delete process.env.DEX_TEST_READ_SUBAGENT_RUNS_FROM_DISK;
      } else {
        process.env.DEX_TEST_READ_SUBAGENT_RUNS_FROM_DISK = previousFlag;
      }
    }
  });
  ...
  it("prefers active runs and can resolve them from persisted registry snapshots", async () => {
    ...
    const resolved = withEnv({ DEX_TEST_READ_SUBAGENT_RUNS_FROM_DISK: "1" }, () =>
  ...
  it("uses isolated temp state when DEX_STATE_DIR is unset in tests", () => {
