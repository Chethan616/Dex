...
describe("resolveReplyToMode", () => {
  ...
  it("uses registered channel threading adapters for runtime reply-mode resolution", () => {
    ...
    expect(resolveReplyToMode({} as DexConfig, "whatsapp", "work", "group")).toBe("first");
    expect(resolveReplyToMode({} as DexConfig, "whatsapp", "default", "group")).toBe("all");
  });
});
