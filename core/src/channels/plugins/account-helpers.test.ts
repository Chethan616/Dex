...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): DexConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as DexConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: DexConfig, expected: string) {
