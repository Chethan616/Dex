...
export const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "DEX_DISABLE_PLUGIN_REGISTRY_MIGRATION";
export const FORCE_PLUGIN_REGISTRY_MIGRATION_ENV = "DEX_FORCE_PLUGIN_REGISTRY_MIGRATION";
...
export type PluginRegistryInstallMigrationParams = LoadInstalledPluginIndexParams &
  InstalledPluginIndexStoreOptions & {
    ...
    readConfig?: () => Promise<DexConfig> | DexConfig;
  };
...
function listConfiguredChannelIds(config: DexConfig): Set<string> {
...
function listConfiguredModelProviderIds(config: DexConfig): Set<string> {
...
export async function migratePluginRegistryForInstall(
  ...
  const config = stripShippedPluginInstallConfigRecords(rawConfig) as DexConfig;
