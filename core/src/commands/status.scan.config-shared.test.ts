...
describe("status.scan.config-shared", () => {
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readBestEffortConfig,
      resolveConfig,
      env: { VITEST: "true", DEX_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "DEX_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove DEX_GATEWAY_TOKEN from the shell, ~/.dex/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${DEX_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a status diagnostic when config uses DEX_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
