...
function expectDexSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = DexSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectDexSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = DexSchema.safeParse(config);
