  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.dex/logs/openclaw.log" },
        hooks: {
          path: "~/.dex/hooks.json5",
        ...
        channels: {
          ...
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.dex/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".dex", "logs", "openclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".dex", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".dex", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".dex", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".dex", "agents", "main"));
