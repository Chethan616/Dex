...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: DexConfig): boolean {
...
function isProviderConfigured(cfg: DexConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: DexConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: DexConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: DexConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: DexConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: DexConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: DexConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: DexConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: DexConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: DexConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: DexConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: DexConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: DexConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: DexConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: DexConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: DexConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: DexConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: DexConfig): boolean {
  return (
...
function hasPluginEntries(cfg: DexConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: DexConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: DexConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: DexConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: DexConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: DexConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: DexConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: DexConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: DexConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
