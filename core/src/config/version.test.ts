...
describe("parseDexVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseDexVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseDexVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseDexVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseDexVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseDexVersion("2026.3")).toBeNull();
    expect(parseDexVersion("latest")).toBeNull();
  });
});

describe("compareDexVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareDexVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareDexVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareDexVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareDexVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareDexVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareDexVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});
