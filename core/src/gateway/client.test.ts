...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.DEX_PROXY_ACTIVE;
    delete process.env.DEX_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with DEX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { DEX_HOME: "/tmp/custom-openclaw-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      DEX_STATE_DIR: "/tmp/openclaw-client-service-state",
