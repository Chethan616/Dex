...
function createSnapshot(config: DexConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: DexConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: DexConfig) {
...
// set DEX_SKIP_CHANNELS / DEX_SKIP_PROVIDERS at module load. When a
