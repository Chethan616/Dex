...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.DEX_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: DexConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: DexConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: DexConfig, requestedAgentId: string) => {
