...
async function startMemoryBackendForTest(cfg: DexConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: DexConfig) {
...
function expectQmdManagerRequests(cfg: DexConfig, agentIds: string[]) {
