...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses DEX_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/openclaw-home"), ".dex", "sessions", "sess-1.jsonl"),
    );
  });
});
