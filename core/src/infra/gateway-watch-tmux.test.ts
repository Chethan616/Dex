...
describe("gateway-watch tmux wrapper", () => {
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      env: {
        ...
        DEX_GATEWAY_RESTART_TRACE: "1",
        DEX_GATEWAY_STARTUP_TRACE: "1",
        DEX_PROFILE: "Dev Profile",
        DEX_TRACE_SYNC_IO: "0",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
    ...
    expect(command).toContain("'DEX_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'DEX_GATEWAY_WATCH_SESSION=openclaw-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'DEX_GATEWAY_PORT=19001'");
    expect(command).toContain("'DEX_GATEWAY_RESTART_TRACE=1'");
    expect(command).toContain("'DEX_GATEWAY_STARTUP_TRACE=1'");
    expect(command).toContain("'DEX_PROFILE=Dev Profile'");
    expect(command).toContain("'DEX_TRACE_SYNC_IO=0'");
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain("'DEX_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'");
    expect(command).toContain("'DEX_RUN_NODE_CPU_PROF_MAX_FILES=40'");
    expect(command).toContain("'DEX_TRACE_SYNC_IO=0'");
  ...
  it("preserves an explicit benchmark CPU profile retention cap", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { DEX_RUN_NODE_CPU_PROF_MAX_FILES: "8", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync, 1);
    expect(command).toContain("'DEX_RUN_NODE_CPU_PROF_MAX_FILES=8'");
  });
  ...
  it("preserves explicit sync I/O tracing in benchmark mode", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { DEX_TRACE_SYNC_IO: "1", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync, 1);
    expect(command).toContain("'DEX_TRACE_SYNC_IO=1'");
    expect(command).toContain(
      "'DEX_RUN_NODE_OUTPUT_LOG=.artifacts/gateway-watch-profiles/gateway-watch-output.log'",
    );
    expect(command).toContain("'DEX_RUN_NODE_FILTER_SYNC_IO_STDERR=1'");
    expect(stderr.chunks.join("")).toContain(
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--port=19001"],
      cwd: "/repo",
      env: { DEX_PROFILE: "dev", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
