...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".dex");
...
function sharedMainOpsConfig(sharedStorePath: string): DexConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: DexConfig = OPS_WORK_CONFIG) {
