...
type CapabilityProviderCandidate = {
  ...
  isConfigured?: (ctx: { cfg?: DexConfig; agentDir?: string }) => boolean;
};
...
function resolveCurrentDefaultProviderId(cfg?: DexConfig): string {
...
function resolveAutoCapabilityFallbackRefs(params: {
  cfg: DexConfig;
  agentDir?: string;
  listProviders: (cfg?: DexConfig) => CapabilityProviderCandidate[];
}): string[] {
