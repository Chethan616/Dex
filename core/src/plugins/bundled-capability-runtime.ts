...
import type { DexPluginDefinition, DexPluginModule } from "./types.js";
  ...
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        ...
        [`@dexagent/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
}
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    ...
    "@dexagent/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as DexPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getModuleLoader(safeSource)(safeSource) as DexPluginModule;
    } catch (error) {
