...
let tempDirIndex = 0;
const prevBundledDir = process.env.DEX_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.DEX_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".dex", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ DEX_STATE_DIR: stateDir, ...params.env }, () =>
    loadOpenClawPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DEX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.DEX_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.DEX_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
