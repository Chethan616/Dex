...
import { normalizeDexVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeDexVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeDexVersionBase(plugin.version) ??
    plugin.version
  );
}
