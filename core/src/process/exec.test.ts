...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    ...
    expect(resolved.DEX_BASE_ENV).toBe("base");
    expect(resolved.DEX_TEST_ENV).toBe("ok");
    expect(resolved.DEX_TO_REMOVE).toBeUndefined();
    expect(resolved.DEX_CLI).toBe(DEX_CLI_ENV_VALUE);
  });
