...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: DexConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when DEX_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: DexConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from DEX_GATEWAY_TOKEN", () => {
    const cfg: DexConfig = {
