...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "DEX_ENV_TEST_A";
    const keyB = "DEX_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "DEX_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "DEX_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "DEX_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "DEX_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousBundledDir = process.env.DEX_BUNDLED_PLUGINS_DIR;
    process.env.DEX_HOME = "/srv/openclaw-home";
    process.env.DEX_STATE_DIR = "/srv/openclaw-state";
    process.env.DEX_BUNDLED_PLUGINS_DIR = "/srv/openclaw-bundled";
    ...
    try {
      ...
      expect(env.DEX_HOME).toBeUndefined();
      expect(env.DEX_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.DEX_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("DEX_HOME", previousOpenClawHome);
      restoreEnvKey("DEX_STATE_DIR", previousStateDir);
      restoreEnvKey("DEX_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { DEX_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processOpenClawHome: process.env.DEX_HOME,
          processBundledDir: process.env.DEX_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.DEX_BUNDLED_PLUGINS_DIR,
        }),
      );
      ...
      expect(process.env.DEX_HOME).toBe("/srv/openclaw-home");
    } finally {
      restoreEnvKey("DEX_HOME", previousOpenClawHome);
    }
  });
