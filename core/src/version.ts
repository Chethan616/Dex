...
function readInjectedVersion(): string | undefined {
  return typeof __DEX_VERSION__ === "string" ? __DEX_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["DEX_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["DEX_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["DEX_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.DEX_COMPATIBILITY_HOST_VERSION);
...
export const VERSION = resolveBinaryVersion({
  ...
  bundledVersion: process.env.DEX_BUNDLED_VERSION,
});
