...
import { createDexTestInstance, testing } from "./openclaw-test-instance.js";
...
describe("openclaw test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createDexTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".dex"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "openclaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.DEX_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.DEX_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.DEX_SKIP_CRON).toBe("0");
