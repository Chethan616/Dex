...
describe("scripts/dev/ios-pull-gateway-log.sh", () => {
  it("does not bake local device or bundle identifiers into the log pull helper", () => {
    ...
    expect(script).toContain('DEVICE_UDID="${1:-${DEX_IOS_DEVICE_UDID:-}}"');
    expect(script).toContain('BUNDLE_ID="${2:-${DEX_IOS_BUNDLE_ID:-}}"');
    expect(script).toContain('DEST="${3:-${DEX_IOS_GATEWAY_LOG_DEST:-}}"');
