// Ported verbatim from the Flutter app's app/lib/core/dex_taglines.dart,
// which mirrors dex-core CLI banner wording. Same product voice across
// the CLI, the old app, and this one — hand-written, not generated.

export const DEX_TAGLINES: readonly string[] = [
...
/** Stable pick for a seed, so a re-render does not reshuffle the line. */
export function taglineFor(seed: number): string {
  if (DEX_TAGLINES.length === 0) return "";
  const i = Math.abs(Math.floor(seed)) % DEX_TAGLINES.length;
  return DEX_TAGLINES[i]!;
}
