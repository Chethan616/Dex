---
summary: "CLI reference and security model for Conch, the configless-safe setup and repair helper"
read_when:
  - You run openclaw with no command after setup and want to understand Conch
...
Conch is OpenClaw's local setup, repair, and configuration helper. It is
...
with no command starts Conch in an interactive terminal. Running
`openclaw conch` starts the same helper explicitly.

## What Conch shows

On startup, interactive Conch opens the same TUI shell used by
`openclaw tui`, with a Conch chat backend. The chat log starts with a short
greeting:

- when to start Conch
- the model or deterministic planner path Conch is actually using
- config validity and the default agent
- Gateway reachability from the first startup probe
- the next debug action Conch can take
...
Conch uses the same OpenClaw reference discovery as regular agents. In a Git checkout,
...
openclaw conch
openclaw conch --json
openclaw conch --message "models"
openclaw conch --message "validate config"
openclaw conch --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
openclaw conch --message "set default model openai/gpt-5.5" --yes
openclaw onboard --modern
```

Inside the Conch TUI:
...
Conch's startup path is deliberately small. It can run when:
...
after setup, it points to one-shot Conch commands.

## Operations and approval

Conch uses typed operations instead of editing config ad hoc.
...
```text
~/.dex/audit/conch.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`openclaw onboard --modern` starts Conch as the modern onboarding preview.
...
Conch always starts in deterministic mode. For fuzzy commands that the
deterministic parser does not understand, local Conch can make one bounded
...
request into one of Conch's typed commands, then the normal approval and
audit rules apply. Conch prints the model it used and the interpreted
...
Use a natural-language selector to leave Conch and open the normal TUI:
...
agent TUI directly. They do not start Conch.

After switching into the normal TUI, use `/conch` to return to Conch.
You can include a follow-up request:

```text
/conch
/conch restart gateway
```

Agent switches inside the TUI leave a breadcrumb that `/conch` is available.

## Message rescue mode

Message rescue mode is the message-channel entrypoint for Conch. It is for
...
```text
You, in a trusted owner DM: /conch status
OpenClaw: Conch rescue mode. Gateway reachable: no. Config valid: no.
You: /conch restart gateway
OpenClaw: Plan: restart the Gateway. Reply /conch yes to apply.
You: /conch yes
...
/conch create agent work workspace ~/Projects/work
```
...
- Disabled when sandboxing is active. If an agent/session is sandboxed,
  Conch must refuse remote rescue and explain that local CLI repair is
  required.
...
```bash
pnpm test:docker:conch-rescue
```

Configless local planner fallback is covered by:

```bash
pnpm test:docker:conch-planner
```

An opt-in live channel command-surface smoke checks `/conch status` plus a
persistent approval roundtrip through the rescue handler:

```bash
pnpm test:live:conch-rescue-channel
```

Configless setup through explicit Conch commands is covered by:

```bash
pnpm test:docker:conch-first-run
```

That lane starts with an empty state dir, verifies the modern onboard Conch
