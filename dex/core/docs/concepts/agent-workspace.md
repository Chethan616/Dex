...
This is separate from `~/.dex/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.dex/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.dex/workspace`
- If `DEX_PROFILE` is set and not `"default"`, the default becomes `~/.dex/workspace-<profile>`.
- Override in `~/.dex/openclaw.json`:
...
These live under `~/.dex/` and should NOT be committed to the workspace repo:

- `~/.dex/openclaw.json` (config)
- `~/.dex/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.dex/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.dex/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.dex/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.dex/skills/` (managed skills)
...
If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.dex/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.dex/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.dex/openclaw.json`.
  </Step>
  ...
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.dex/agents/<agentId>/sessions/` from the old machine separately.
  </Step>
</Steps>
