...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.dex/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.dex/agents/<agentId>/sessions`.

Auth profiles are **per-agent**. Each agent reads from its own:

```text
~/.dex/agents/<agentId>/agent/auth-profiles.json
```
...
Skills are loaded from each agent workspace plus shared roots such as `~/.dex/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.dex` (or `DEX_STATE_DIR`)
- Workspace: `~/.dex/workspace` (or `~/.dex/workspace-<agentId>`)
- Agent dir: `~/.dex/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.dex/agents/<agentId>/sessions`
...
- Workspace defaults to `~/.dex/workspace` (or `~/.dex/workspace-<profile>` when `DEX_PROFILE` is set).
- State defaults to `~/.dex/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.dex/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.dex/workspace-alex" },
      { id: "mia", workspace: "~/.dex/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.dex/workspace-main" },
          { id: "coding", workspace: "~/.dex/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.dex/workspace-main" },
          { id: "alerts", workspace: "~/.dex/workspace-alerts" },
        ],
      },
      bindings: [
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.dex/workspace-home",
            agentDir: "~/.dex/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.dex/workspace-work",
            agentDir: "~/.dex/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.dex/credentials/whatsapp/personal
              // authDir: "~/.dex/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.dex/credentials/whatsapp/biz
              // authDir: "~/.dex/credentials/whatsapp/biz",
            },
          },
        },
      },
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.dex/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.dex/workspace-opus",
