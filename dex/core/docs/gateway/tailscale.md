...
- `token` (default when `DEX_GATEWAY_TOKEN` is set)
- `password` (shared secret via `DEX_GATEWAY_PASSWORD` or config)
...
Prefer `DEX_GATEWAY_PASSWORD` over committing a password to disk.
