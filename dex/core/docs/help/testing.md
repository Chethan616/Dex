...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `DEX_LIVE_MODEL_FILE_PROBE=0` or
    `DEX_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `DEX_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `DEX_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 DEX_LIVE_CODEX_HARNESS_MCP_PROBE=0 DEX_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 DEX_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `DEX_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
...
commit, then pull it with `DEX_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `DEX_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
...
- `pnpm test:docker:npm-telegram-live`
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout; the
    ...
    under test.
  - Defaults to `DEX_NPM_TELEGRAM_PACKAGE_SPEC=openclaw@beta`; set
    `DEX_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/openclaw-current.tgz` or
    `DEX_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
  ...
  - Uses the same Telegram env credentials or Convex credential source as
    ...
    `DEX_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `DEX_QA_CONVEX_SITE_URL` and the role secret. If
    `DEX_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
  ...
  - `DEX_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `DEX_QA_CREDENTIAL_ROLE` for this lane only.
...
`source=trusted-url` reads `.github/package-trusted-sources.json` from the trusted workflow ref and does not accept URL credentials or a workflow-input private-network bypass. If the named policy declares bearer auth, configure the fixed `DEX_TRUSTED_PACKAGE_TOKEN` secret.
...
- `pnpm test:parallels:npm-update`
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `DEX_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
...
- `pnpm openclaw qa telegram`
  ...
  - Requires `DEX_QA_TELEGRAM_GROUP_ID`, `DEX_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `DEX_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `DEX_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
...
- `pnpm openclaw qa mantis telegram-desktop-builder`
  ...
  - Defaults to `--credential-source convex` so workflows only need the Convex broker secret. Use `--credential-source env` with the same `DEX_QA_TELEGRAM_*` variables as `pnpm openclaw qa telegram`.
...
When `--credential-source convex` (or `DEX_QA_CREDENTIAL_SOURCE=convex`) is enabled for
...
- `DEX_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `DEX_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `DEX_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `DEX_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `DEX_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `DEX_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `DEX_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `DEX_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `DEX_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `DEX_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `DEX_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`DEX_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`DEX_QA_CONVEX_SECRET_MAINTAINER` specifically.
...
Default endpoint contract (`DEX_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `DEX_TEST_CHANGED_BROAD=1 pnpm test:changed`.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `DEX_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
    - `scripts/run-vitest.mjs` terminates explicit non-watch Vitest runs after
      5 minutes with no stdout or stderr output. Set
      `DEX_VITEST_NO_OUTPUT_TIMEOUT_MS=0` to disable the watchdog for an
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `DEX_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `DEX_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `DEX_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `DEX_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `DEX_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Useful overrides:
  - `DEX_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `DEX_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `DEX_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.dex`.
- Set `DEX_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `DEX_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `DEX_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `DEX_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `DEX_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners keep their own practical caps where needed:
  ...
  `test:docker:live-gateway` defaults to `DEX_LIVE_GATEWAY_SMOKE=1`,
  `DEX_LIVE_GATEWAY_MAX_MODELS=8`,
  `DEX_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `DEX_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Set `DEX_LIVE_MAX_MODELS`
  or the gateway env vars when you explicitly want a smaller cap or larger scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs OpenClaw once as an npm tarball through `scripts/package-openclaw-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `DEX_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `DEX_DOCKER_ALL_LIVE_LIMIT=9`, `DEX_DOCKER_ALL_NPM_LIMIT=10`, and `DEX_DOCKER_ALL_SERVICE_LIMIT=7`; tune `DEX_DOCKER_ALL_WEIGHT_LIMIT` or `DEX_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale OpenClaw E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `DEX_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Docker/Bash E2E lanes that install the packed OpenClaw tarball through `scripts/lib/openclaw-e2e-instance.sh` cap `npm install` at `DEX_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed OpenClaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `DEX_CURRENT_PACKAGE_TGZ=/path/to/openclaw-*.tgz`, skip the host rebuild with `DEX_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `DEX_NPM_ONBOARD_CHANNEL=discord` or `DEX_NPM_ONBOARD_CHANNEL=slack`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `openclaw infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `OPENCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/openclaw-*.tgz`, skip the host build with `OPENCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `DEX_BUN_GLOBAL_SMOKE_DIST_IMAGE=openclaw-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `DEX_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `DEX_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `DEX_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `DEX_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `DEX_PLUGINS_E2E_CLAWHUB_SPEC` and `DEX_PLUGINS_E2E_CLAWHUB_ID`. Without `DEX_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
...
```bash
DEX_DOCKER_E2E_IMAGE=openclaw-docker-e2e-functional:local pnpm test:docker:e2e-build
DEX_DOCKER_E2E_IMAGE=openclaw-docker-e2e-functional:local DEX_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `DEX_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `DEX_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
They also set `DEX_SKIP_CHANNELS=1` so gateway live probes do not start
...
`DEX_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
...
environment, staged auth profiles, or an explicit `DEX_PROFILE_FILE`.
...
- `DEX_CONFIG_DIR=...` (default: `~/.dex`) mounted to `/home/node/.openclaw`
- `DEX_WORKSPACE_DIR=...` (default: `~/.dex/workspace`) mounted to `/home/node/.openclaw/workspace`
- `DEX_PROFILE_FILE=...` mounted and sourced before running tests
- `OPENCLAW_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `DEX_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `DEX_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/openclaw/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `DEX_LIVE_PROVIDERS` / `DEX_LIVE_GATEWAY_PROVIDERS`
...
- `DEX_LIVE_GATEWAY_MODELS=...` / `DEX_LIVE_MODELS=...` to narrow the run
- `DEX_LIVE_GATEWAY_PROVIDERS=...` / `DEX_LIVE_PROVIDERS=...` to filter providers in-container
- `DEX_SKIP_DOCKER_BUILD=1` to reuse an existing `openclaw:local-live` image for reruns that do not need a rebuild
- `DEX_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `DEX_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
