...
3. Set the required **Variables** (at least `DEX_GATEWAY_PORT` and `DEX_GATEWAY_TOKEN`).
4. Enable **HTTP Proxy** on port `8080`.
5. Open `https://<your-railway-domain>/openclaw` and connect using the configured shared secret. This template uses `DEX_GATEWAY_TOKEN` by default; if you replace it with password auth, use that password instead.
...
- `DEX_GATEWAY_PORT=8080` (required — must match the port in Public Networking)
- `DEX_GATEWAY_TOKEN` (required; treat as an admin secret)
- `DEX_STATE_DIR=/data/.openclaw` (recommended)
- `DEX_WORKSPACE_DIR=/data/workspace` (recommended)
