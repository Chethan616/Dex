...
services:
  - type: web
    ...
    envVars:
      - key: DEX_GATEWAY_PORT
        value: "8080"
      - key: DEX_STATE_DIR
        value: /data/.openclaw
      - key: DEX_WORKSPACE_DIR
        value: /data/workspace
      - key: DEX_GATEWAY_TOKEN
...
`DEX_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
- Missing `DEX_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `DEX_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
