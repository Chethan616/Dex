...
- One-off slug generation and Atlas planner runs use SQLite transcript rows
...
- Atlas audit entries now use core SQLite plugin state instead of
  `audit/atlas.jsonl`. Doctor imports the legacy JSONL audit log and
...
- Atlas rescue pending approvals now use core SQLite plugin state instead
  of `atlas/rescue-pending/*.json`. Doctor imports legacy pending approval
