...
```sh
DEX_LIVE_TEST=1 DEX_LIVE_CACHE_TEST=1 pnpm test:live:cache
```
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.dex/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$DEX_STATE_DIR/logs/cache-trace.jsonl`
...
- `DEX_CACHE_TRACE=1` enables cache tracing.
- `DEX_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `DEX_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `DEX_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `DEX_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
