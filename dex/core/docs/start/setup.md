...
- **Tailoring lives outside the repo:** keep your config and workspace in `~/.dex/openclaw.json` and `~/.dex/workspace/` so repo updates don't touch them.
...
- **Config:** `~/.dex/openclaw.json` (JSON/JSON5-ish)
- **Workspace:** `~/.dex/workspace` (skills, prompts, memories; make it a private git repo)
...
```bash
node dex.mjs gateway --port 18789 --verbose
```
...
`DEX_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch` to keep an interactive run
...
`DEX_GATEWAY_WATCH_AUTO_DOCTOR=0` to disable that dev-only repair pass.
...
- **Where state lives:**
  - Channel/provider state: `~/.dex/credentials/`
...
- Keep `~/.dex/workspace` and `~/.dex/` as "your stuff"; don't put personal prompts/config into the `openclaw` repo.
