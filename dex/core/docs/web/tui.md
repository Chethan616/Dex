...
- After a config file has authored settings, `openclaw` and `openclaw conch` also use this TUI shell, with Conch as the local setup and repair chat backend.
...
- Local shell commands receive `DEX_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `DEX_THEME=light` before launching `openclaw tui`.
- To force the original dark palette instead, set `DEX_THEME=dark`.
