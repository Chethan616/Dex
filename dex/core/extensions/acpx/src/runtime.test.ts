...
const DOCUMENTED_DEX_BRIDGE_COMMAND =
  "env DEX_HIDE_BANNER=1 DEX_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:18789 --token-file ~/.dex/gateway.token --session agent:main:main";
...
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${DEX_ACPX_LEASE_ID_ARG} lease-close ${DEX_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(launchCommands[0]).toContain("DEX_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("DEX_GATEWAY_INSTANCE_ID=gateway-test");
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${DEX_ACPX_LEASE_ID_ARG} lease-old ${DEX_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${DEX_ACPX_LEASE_ID_ARG} lease-current ${DEX_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("uses the bridge-safe delegate for documented env-wrapped openclaw bridge commands", async () => {
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "openclaw" ? DOCUMENTED_DEX_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "openclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        agentCommand: DOCUMENTED_DEX_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
