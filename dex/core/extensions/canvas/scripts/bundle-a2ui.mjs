...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.DEX_SPARSE_PROFILE || process.env.DEX_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because DEX_A2UI_SKIP_MISSING=1 or DEX_SPARSE_PROFILE is set.",
      );
      return;
    }
