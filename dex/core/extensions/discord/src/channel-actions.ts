...
import type { DiscordActionConfig, DexConfig } from "openclaw/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: DexConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: DexConfig) {
