...
function hasConfiguredMatrixChannel(cfg: DexConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: DexConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: DexConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export async function collectMatrixInstallPathWarnings(cfg: DexConfig): Promise<string[]> {
...
export async function cleanStaleMatrixPluginConfig(cfg: DexConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
