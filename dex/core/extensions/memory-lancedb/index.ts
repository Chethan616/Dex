...
import { definePluginEntry, type DexPluginApi } from "./api.js";
import {
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as DexConfig;
...
function createEmbeddings(api: DexPluginApi, cfg: MemoryConfig): Embeddings {
