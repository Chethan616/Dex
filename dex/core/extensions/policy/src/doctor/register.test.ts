...
function cfgWithPolicy(settings: Record<string, unknown> = {}): DexConfig {
  return {
    plugins: {
      entries: {
        policy: {
...
function ctx(configPath: string, cfg: DexConfig = {}): HealthCheckContext {
  return {
    mode: "lint",
    runtime: {
      log() {},
...
function repairCtx(configPath: string, cfg: DexConfig = {}): HealthRepairContext {
  return {
    ...ctx(configPath, cfg),
    mode: "fix",
  };
