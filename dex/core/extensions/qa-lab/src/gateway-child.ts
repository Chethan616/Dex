...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    DEX_HOME: params.homeDir,
    DEX_CONFIG_PATH: params.configPath,
    DEX_STATE_DIR: params.stateDir,
    DEX_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    DEX_NO_RESPAWN: "1",
    DEX_TEST_FAST: "1",
    DEX_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    DEX_QA_PARENT_PID: String(process.pid),
    DEX_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { DEX_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    DEX_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    DEX_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { DEX_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { DEX_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const keepTemp = process.env.DEX_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
