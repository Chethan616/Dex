...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => DexConfig>());

vi.mock("openclaw/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): DexConfig {
  return shape as unknown as DexConfig;
}
