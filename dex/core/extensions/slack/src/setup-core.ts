...
function enableSlackAccount(cfg: DexConfig, accountId: string): DexConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: DexConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: DexConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: DexConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: DexConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: DexConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: DexConfig; accountId: string }) =>
    ...
    disable: (cfg: DexConfig) => setSetupChannelEnabled(cfg, channel, false),
