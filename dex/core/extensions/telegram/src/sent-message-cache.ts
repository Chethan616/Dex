...
function resolveSentMessageStorePath(cfg?: Pick<DexConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<DexConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<DexConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<DexConfig, "session">): SentMessageStore {
