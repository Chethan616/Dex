...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: DexConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: DexConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: DexConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: DexConfig, accountId: string): Promise<boolean> {
