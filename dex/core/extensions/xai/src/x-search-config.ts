...
function resolveLegacyXSearchConfig(config?: DexConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: DexConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: DexConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: DexConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: DexConfig): JsonRecord | undefined {
