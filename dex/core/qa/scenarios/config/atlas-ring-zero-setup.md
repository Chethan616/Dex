# Atlas ring-zero setup

```yaml qa-scenario
id: atlas-ring-zero-setup
title: Atlas ring-zero setup
surface: config
coverage:
  primary:
    - config.atlas-setup
  secondary:
    ...
    - agents.create
objective: Verify Atlas can bootstrap a fresh OpenClaw config, set the default model, create an agent, configure Discord through a SecretRef, validate config, and leave an audit trail.
successCriteria:
  - Atlas reports missing config in an empty state dir.
  - Atlas setup writes a workspace and default model.
  - Atlas creates a non-main agent with its own workspace and model.
  - Atlas enables the Discord plugin before writing Discord channel config.
  - Atlas configures Discord through an env SecretRef without persisting the raw token.
...
codeRefs:
  - src/atlas/operations.ts
  - scripts/e2e/atlas-first-run-spec.json
  - scripts/e2e/atlas-first-run-docker-client.ts
  - extensions/qa-lab/src/suite-runtime-agent-process.ts
execution:
  kind: flow
  summary: Drive the public Atlas CLI in an isolated fresh state dir and verify setup/model/agent/Discord/audit results.
  config:
    specPath: scripts/e2e/atlas-first-run-spec.json
```

```yaml qa-flow
steps:
  - name: bootstraps config through Atlas CLI
    actions:
      - set: setupSpec
        value:
      ...
      - assert:
          ...
          message:
            expr: "`fresh Atlas overview did not report missing config: ${overviewOutput}`"
      - assert:
          expr: 'String(overviewOutput).includes(''Next: run "setup" to create a starter config'')'
          message:
            expr: "`fresh Atlas overview did not recommend setup: ${overviewOutput}`"
      - forEach:
          items:
          ...
          actions:
            - call: runQaCli
              ...
              args:
                - ref: env
                - expr: "['atlas', ...(commandStep.approve ? ['--yes'] : []), '-m', renderCommand(commandStep.message)]"
                - timeoutMs: 60000
                  env:
                    ref: atlasEnv
            - assert:
                ...
                message:
                  expr: "`Atlas command ${commandStep.id} did not produce ${commandStep.expectOutput}: ${commandOutput}`"
      - set: writtenConfig
        value:
      ...
      - assert:
          ...
          message: Atlas persisted the raw Discord token.
      - set: auditText
        value:
          expr: "await fs.readFile(path.join(stateDir, 'audit', 'atlas.jsonl'), 'utf8')"
      - forEach:
          items:
