...
| --------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| `<project>/.env`            | **Docker infra** — image, ports, gateway token   | `DEX_GATEWAY_TOKEN`, `DEX_IMAGE`, `DEX_GATEWAY_PORT`, `DEX_AUTH_PROFILE_SECRET_DIR` |
| `~/.dex/.env`          | **Secrets** — API keys and bot tokens            | `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `TELEGRAM_BOT_TOKEN`                                             |
| `~/.dex/openclaw.json` | **Behavior config** — models, channels, policies | Model selection, WhatsApp allowlists, agent settings                                                    |

**Do NOT** put API keys or bot tokens in `openclaw.json`. Use `~/.dex/.env` for all secrets.
...
- Sets up `~/.dex` directories if they don't exist
...
- `DEX_IMAGE_APT_PACKAGES` — extra apt packages to install (e.g. `ffmpeg`); also accepts legacy `DEX_DOCKER_APT_PACKAGES`
- `DEX_IMAGE_PIP_PACKAGES` — extra Python packages to install (e.g. `requests==2.32.5`); pin versions and use only package indexes you trust
- `DEX_INSTALL_BROWSER=1` — pre-install Chromium for browser automation (adds ~300MB, but skips the 60-90s Playwright install on each container start)
...
volumes:
  ...
  - ${DEX_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/openclaw
```

This means:

- `~/.dex/.env` is available inside the container at `/home/node/.openclaw/.env` — OpenClaw loads it automatically as the global env fallback
- `~/.dex/openclaw.json` is available at `/home/node/.openclaw/openclaw.json` — the gateway watches it and hot-reloads most changes
- `~/.dex-auth-profile-secrets` is available at `/home/node/.config/openclaw` — OpenClaw stores the auth-profile encryption key there
...
The project `.env` feeds Docker Compose directly (gateway token, image name, ports). The `~/.dex/.env` feeds the OpenClaw process inside the container.
...
```bash
DEX_CONFIG_DIR=/Users/you/.openclaw
DEX_WORKSPACE_DIR=/Users/you/.openclaw/workspace
DEX_GATEWAY_PORT=18789
DEX_BRIDGE_PORT=18790
DEX_GATEWAY_BIND=lan
DEX_GATEWAY_TOKEN=<generated-by-docker-setup>
DEX_AUTH_PROFILE_SECRET_DIR=/Users/you/.openclaw-auth-profile-secrets
DEX_IMAGE=openclaw:local
```
...
3. **`~/.dex/.env`** — global secrets (API keys, bot tokens)
4. **`openclaw.json` `env` block** — inline vars, applied only if still missing
5. **Shell env import** — optional login-shell scrape (`DEX_LOAD_SHELL_ENV=1`)
