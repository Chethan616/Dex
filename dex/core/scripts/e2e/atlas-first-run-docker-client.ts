// Atlas first-run Docker harness.
...
async function readFirstRunSpec(): Promise<AtlasFirstRunSpec> {
  return JSON.parse(
    await fs.readFile(
      path.join(process.cwd(), "scripts", "e2e", "atlas-first-run-spec.json"),
      "utf8",
    ),
  ) as AtlasFirstRunSpec;
}
...
async function main() {
  ...
  const tempState = await createE2eStateDir("openclaw-atlas-first-run-");
  ...
  assert(
    shouldStartAtlasForModernOnboard(["node", "openclaw", "onboard", "--modern"]),
    "modern onboard invocation did not route to Atlas",
  );
  ...
  await runAtlas({ message: "overview", interactive: false }, overviewRuntime.runtime);
  ...
  for (const command of spec.commands) {
    ...
    assert(
      ...
      `Atlas first-run command ${command.id} did not apply: ${output}`,
    );
  }
  ...
  assert(reef, "Atlas did not create reef agent");
  assert(reef.workspace === spec.dockerAgentWorkspace, "Atlas did not write reef workspace");
  assert(reef.model === spec.model, "Atlas did not write reef model");
  assert(config.plugins?.allow?.includes("discord"), "Atlas did not allow Discord plugin");
  assert(
    config.plugins?.entries?.discord?.enabled === true,
    "Atlas did not enable Discord plugin entry",
  );
  assert(config.channels?.discord?.enabled === true, "Atlas did not enable Discord");
  ...
  assert(
    ...
    "Atlas did not write Discord token SecretRef",
  );
  assert(
    !JSON.stringify(config.channels.discord).includes(spec.discordToken),
    "Atlas persisted the raw Discord token",
  );
  ...
  console.log("Atlas first-run Docker E2E passed");
}
