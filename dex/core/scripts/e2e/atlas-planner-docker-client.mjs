...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-atlas-planner-")));
  ...
  assertOutputIncludes(
    output,
    "[atlas] planner: claude-cli/claude-opus-4-8",
  ...
  assertOutputIncludes(
    output,
    "[atlas] interpreted: set default model openai/gpt-5.2",
    "planner command was not interpreted",
  );
  assertOutputIncludes(
    output,
    "[atlas] done: config.setDefaultModel",
  ...
  console.log("Atlas planner Docker E2E passed");
  process.exit(0);
}
