// Atlas rescue-message Docker harness.
...
import { handleAtlasCommand } from "../../dist/auto-reply/reply/commands-atlas.js";
import { clearConfigCache } from "../../dist/config/config.js";
import type { DexConfig } from "../../dist/config/types.openclaw.js";
import { runAtlasRescueMessage } from "../../dist/atlas/rescue-message.js";
import { createE2eStateDir } from "./lib/temp-state-dir.ts";

type CommandResult = Awaited<ReturnType<typeof handleAtlasCommand>>;
...
function makeParams(commandBody: string, cfg: DexConfig, isGroup = false) {
  return {
    ...
    agentId: "default",
    isGroup,
  } as Parameters<typeof handleAtlasCommand>[0];
}

async function invoke(commandBody: string, cfg: DexConfig, isGroup = false): Promise<string> {
  const result: CommandResult = await handleAtlasCommand(
...
async function main() {
  const tempState = await createE2eStateDir("openclaw-atlas-");
  ...
  const denied = await invoke("/atlas status", {
  ...
  const cfg: DexConfig = {};
  const refusedTui = await invoke("/atlas talk to agent", cfg);
  assert(
    refusedTui.includes("cannot open the local TUI"),
    "remote rescue TUI handoff was not refused",
  );

  const plan = await invoke("/atlas set default model openai/gpt-5.2", cfg);
  assert(
    plan.includes("Reply /atlas yes to apply"),
    "persistent change did not require approval",
  );
  const applied = await invoke("/atlas yes", cfg);
  assert(applied.includes("Default model: openai/gpt-5.2"), "approved change did not apply");

  const configValid = await invoke("/atlas validate config", cfg);
  assert(configValid.includes("Config valid:"), "config validation did not report valid config");

  const configSetPlan = await invoke("/atlas config set gateway.port 19001", cfg);
  assert(
    configSetPlan.includes("Reply /atlas yes to apply"),
    "generic config set did not require approval",
  );
  const configSetApplied = await invoke("/atlas yes", cfg);
  assert(configSetApplied.includes("[atlas] done: config.set"), "generic config set failed");

  const refPlan = await invoke(
    "/atlas config set-ref gateway.auth.token env DEX_GATEWAY_TOKEN",
    cfg,
  );
  assert(
    refPlan.includes("Reply /atlas yes to apply"),
    "SecretRef set did not require approval",
  );
  const refApplied = await invoke("/atlas yes", cfg);
  assert(refApplied.includes("[atlas] done: config.setRef"), "SecretRef set failed");

  const agentPlan = await invoke("/atlas create agent work workspace /tmp/openclaw-work", cfg);
  assert(
    agentPlan.includes("Reply /atlas yes to apply"),
    "agent creation did not require approval",
  );
  const agentApplied = await invoke("/atlas yes", cfg);
  assert(agentApplied.includes("[atlas] done: agents.create"), "agent creation did not apply");

  const setupPlan = await invoke(
    "/atlas setup workspace /tmp/openclaw-setup model openai/gpt-5.2",
    cfg,
  );
  assert(setupPlan.includes("Reply /atlas yes to apply"), "setup did not require approval");
  const setupApplied = await invoke("/atlas yes", cfg);
  assert(setupApplied.includes("[atlas] done: atlas.setup"), "setup did not apply");

  const gatewayRestarts: string[] = [];
  const gatewayCommand = makeParams("/atlas restart gateway", cfg).command;
  const gatewayPlan = await runAtlasRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/atlas restart gateway",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    gatewayPlan?.includes("Reply /atlas yes to apply"),
    "gateway restart did not require approval",
  );
  const gatewayApplied = await runAtlasRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/atlas yes",
    agentId: "default",
    isGroup: false,
    deps: {
      runGatewayRestart: async () => {
  ...
  assert(
    gatewayApplied?.includes("[atlas] done: gateway.restart"),
  ...
  const doctorCommand = makeParams("/atlas doctor fix", cfg).command;
  const doctorPlan = await runAtlasRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/atlas doctor fix",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    doctorPlan?.includes("Reply /atlas yes to apply"),
    "doctor fix did not require approval",
  );
  const doctorApplied = await runAtlasRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/atlas yes",
    agentId: "default",
  ...
  assert(doctorApplied?.includes("[atlas] done: doctor.fix"), "doctor fix did not apply");
  ...
  const auditPath = path.join(stateDir, "audit", "atlas.jsonl");
  ...
  assert(
    audits.some((audit) => audit.operation === "atlas.setup"),
    "setup audit missing",
  );
  ...
  console.log("Atlas rescue Docker E2E passed");
}
