...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-conch-planner-")));
  ...
  assertOutputIncludes(
    output,
    "[conch] planner: claude-cli/claude-opus-4-8",
  ...
  assertOutputIncludes(
    output,
    "[conch] interpreted: set default model openai/gpt-5.2",
    "planner command was not interpreted",
  );
  assertOutputIncludes(
    output,
    "[conch] done: config.setDefaultModel",
  ...
  console.log("Conch planner Docker E2E passed");
  process.exit(0);
}
