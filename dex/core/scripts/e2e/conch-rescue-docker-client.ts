// Conch rescue-message Docker harness.
...
import { handleConchCommand } from "../../dist/auto-reply/reply/commands-conch.js";
import { clearConfigCache } from "../../dist/config/config.js";
import type { DexConfig } from "../../dist/config/types.openclaw.js";
import { runConchRescueMessage } from "../../dist/conch/rescue-message.js";
import { createE2eStateDir } from "./lib/temp-state-dir.ts";

type CommandResult = Awaited<ReturnType<typeof handleConchCommand>>;
...
function makeParams(commandBody: string, cfg: DexConfig, isGroup = false) {
  return {
    ...
    agentId: "default",
    isGroup,
  } as Parameters<typeof handleConchCommand>[0];
}

async function invoke(commandBody: string, cfg: DexConfig, isGroup = false): Promise<string> {
  const result: CommandResult = await handleConchCommand(
...
async function main() {
  const tempState = await createE2eStateDir("openclaw-conch-");
  ...
  const denied = await invoke("/conch status", {
  ...
  const cfg: DexConfig = {};
  const refusedTui = await invoke("/conch talk to agent", cfg);
  assert(
    refusedTui.includes("cannot open the local TUI"),
    "remote rescue TUI handoff was not refused",
  );

  const plan = await invoke("/conch set default model openai/gpt-5.2", cfg);
  assert(
    plan.includes("Reply /conch yes to apply"),
    "persistent change did not require approval",
  );
  const applied = await invoke("/conch yes", cfg);
  assert(applied.includes("Default model: openai/gpt-5.2"), "approved change did not apply");

  const configValid = await invoke("/conch validate config", cfg);
  assert(configValid.includes("Config valid:"), "config validation did not report valid config");

  const configSetPlan = await invoke("/conch config set gateway.port 19001", cfg);
  assert(
    configSetPlan.includes("Reply /conch yes to apply"),
    "generic config set did not require approval",
  );
  const configSetApplied = await invoke("/conch yes", cfg);
  assert(configSetApplied.includes("[conch] done: config.set"), "generic config set failed");

  const refPlan = await invoke(
    "/conch config set-ref gateway.auth.token env DEX_GATEWAY_TOKEN",
    cfg,
  );
  assert(
    refPlan.includes("Reply /conch yes to apply"),
    "SecretRef set did not require approval",
  );
  const refApplied = await invoke("/conch yes", cfg);
  assert(refApplied.includes("[conch] done: config.setRef"), "SecretRef set failed");

  const agentPlan = await invoke("/conch create agent work workspace /tmp/openclaw-work", cfg);
  assert(
    agentPlan.includes("Reply /conch yes to apply"),
    "agent creation did not require approval",
  );
  const agentApplied = await invoke("/conch yes", cfg);
  assert(agentApplied.includes("[conch] done: agents.create"), "agent creation did not apply");

  const setupPlan = await invoke(
    "/conch setup workspace /tmp/openclaw-setup model openai/gpt-5.2",
    cfg,
  );
  assert(setupPlan.includes("Reply /conch yes to apply"), "setup did not require approval");
  const setupApplied = await invoke("/conch yes", cfg);
  assert(setupApplied.includes("[conch] done: conch.setup"), "setup did not apply");

  const gatewayRestarts: string[] = [];
  const gatewayCommand = makeParams("/conch restart gateway", cfg).command;
  const gatewayPlan = await runConchRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/conch restart gateway",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    gatewayPlan?.includes("Reply /conch yes to apply"),
    "gateway restart did not require approval",
  );
  const gatewayApplied = await runConchRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/conch yes",
    agentId: "default",
    isGroup: false,
    deps: {
      runGatewayRestart: async () => {
  ...
  assert(
    gatewayApplied?.includes("[conch] done: gateway.restart"),
  ...
  const doctorCommand = makeParams("/conch doctor fix", cfg).command;
  const doctorPlan = await runConchRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/conch doctor fix",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    doctorPlan?.includes("Reply /conch yes to apply"),
    "doctor fix did not require approval",
  );
  const doctorApplied = await runConchRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/conch yes",
    agentId: "default",
  ...
  assert(doctorApplied?.includes("[conch] done: doctor.fix"), "doctor fix did not apply");
  ...
  assert(
    audits.some((audit) => audit.operation === "conch.setup"),
    "setup audit missing",
  );
  ...
  console.log("Conch rescue Docker E2E passed");
}
