...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "DEX_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("DEX_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.DEX_MCP_CODE_MODE_MODEL?.trim() || "openclaw/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing DEX_STATE_DIR");
