...
if (Test-Path $donePath) {
  ...
  "__DEX_BACKGROUND_EXIT__:$backgroundExit"
  ...
  exit 0
      ...
      const offsetMatch = poll.stdout.match(/__DEX_LOG_OFFSET__:(\d+)/);
      if (offsetMatch) {
        lastLogOffset = Number(offsetMatch[1]);
      }
      const lengthMatch = poll.stdout.match(/__DEX_LOG_LENGTH__:(\d+)/);
      const logLength = lengthMatch ? Number(lengthMatch[1]) : lastLogOffset;
      if (poll.stdout.includes("__DEX_BACKGROUND_DONE__")) {
        doneSeen = true;
        ...
        const exitMatch = poll.stdout.match(/__DEX_BACKGROUND_EXIT__:(\S+)/);
...
if (Test-Path $pidPath) {
  ...
  if ($backgroundPid) {
    Stop-DexBackgroundProcessTree ([int]$backgroundPid)
  }
}
`
    : "";
...
export class LinuxGuest {
  ...
  exec(args: string[], options: GuestExecOptions = {}): string {
    const result = run(
      "prlctl",
      ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "DEX_ALLOW_ROOT=1", ...args],
      {
        check: false,
        input: options.input,
        quiet: true,
