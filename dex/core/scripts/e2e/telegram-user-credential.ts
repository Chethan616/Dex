...
const COMMAND_TIMEOUT_MS = optionalPositiveInteger(
  process.env.DEX_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS?.trim(),
  120_000,
);
const BROKER_TIMEOUT_MS = optionalPositiveInteger(
  process.env.DEX_TELEGRAM_USER_CREDENTIAL_BROKER_TIMEOUT_MS?.trim(),
  30_000,
);
...
async function resolveConvexLeaseConfig(opts: Map<string, string>) {
  ...
  const siteUrl =
    opts.get("site-url") ||
    process.env.DEX_QA_CONVEX_SITE_URL?.trim() ||
    fileEnv.DEX_QA_CONVEX_SITE_URL;
  const token =
    opts.get("ci-secret") ||
    process.env.DEX_QA_CONVEX_SECRET_CI?.trim() ||
    fileEnv.DEX_QA_CONVEX_SECRET_CI;
  if (!siteUrl) {
    throw new Error("Missing DEX_QA_CONVEX_SITE_URL.");
  }
  if (!token) {
    throw new Error("Missing DEX_QA_CONVEX_SECRET_CI.");
  }
  return {
    siteUrl,
    token,
    leaseTtlMs: optionalPositiveInteger(
      opts.get("lease-ttl-ms") ||
        process.env.DEX_QA_CREDENTIAL_LEASE_TTL_MS?.trim() ||
        fileEnv.DEX_QA_CREDENTIAL_LEASE_TTL_MS,
      20 * 60 * 1_000,
    ),
    heartbeatIntervalMs: optionalPositiveInteger(
      opts.get("heartbeat-interval-ms") ||
        process.env.DEX_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS?.trim() ||
        fileEnv.DEX_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS,
      30_000,
    ),
    ownerId:
      opts.get("owner-id") ||
...
async function createTelegramUserPayload(opts: Map<string, string>) {
  ...
  const sutToken =
    process.env.DEX_QA_TELEGRAM_SUT_BOT_TOKEN?.trim() ||
  ...
  const groupId =
    process.env.DEX_QA_TELEGRAM_GROUP_ID?.trim() ||
