...
const skipPackValidationEnv = "DEX_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "DEX_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.bin?.openclaw !== "dex.mjs") {
    errors.push(
      `package.json bin.openclaw must be "dex.mjs"; found "${pkg.bin?.openclaw ?? ""}".`,
    );
  }
