...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DexConfig = {
      agents: {
        defaults: {
          model: {
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgStringModel: DexConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: DexConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgInheritDefaultsWithoutAgentModel: DexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: DexConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses DEX_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("DEX_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as DexConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".dex", "workspace"));
  });

  it("uses DEX_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses DEX_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("DEX_HOME", home);
    // Clear state dir so it falls back to DEX_HOME
    vi.stubEnv("DEX_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as DexConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".dex", "agents", "main", "agent"));
  });
