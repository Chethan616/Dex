...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.dex/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "dex doctor" and "dex approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
