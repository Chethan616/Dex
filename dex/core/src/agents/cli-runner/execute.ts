...
const CLI_BACKEND_PRESERVE_ENV = "DEX_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.DEX_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.DEX_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.DEX_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.DEX_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.DEX_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.DEX_MCP_TOKEN || env.DEX_MCP_SESSION_KEY || env.DEX_MCP_AGENT_ID) {
