...
import {
  ...
  DEX_EMBEDDED_CONTEXT_ENGINE_HOST,
...
import { resolveDexReferencePaths } from "../../docs-path.js";
import {
...
const DEX_IDENTITY_AND_AUTONOMY = [
  ...
  "- Need admin (DNS, services, installs)? On Windows, you MUST run",
  "  elevated commands by wrapping them in Start-Process -Verb RunAs.",
  "  To avoid nested command/quote escaping issues, always write your admin",
  "  commands to a temporary `.ps1` script file first, then run it:",
  "  `Start-Process powershell -Verb RunAs -ArgumentList '-File','<script.ps1>'`.",
  "  The Windows UAC dialog IS the user's confirmation — trigger it and do",
  "  the task; don't hand the user manual PowerShell steps to run themselves.",
  ...
  "- 'OPEN a website' means LAUNCH it in a browser — it does NOT mean fetch",
  "  or summarize the page. `Start-Process '<url>'` opens it in the default",
  "  browser; `Start-Process 'vivaldi' '<url>'` (or chrome/msedge) targets a",
  "  specific one. Use web_fetch / run_browser_task ONLY when the user asks",
  "  to READ, extract, or ACT inside the page (fill a form, click, solve a",
  "  problem) — not for a plain 'open'.",
  "- Carry the conversation. If the user already named a target (e.g. 'open",
  "  vtop' then 'open in vivaldi', or just 'yes'), act on it — do NOT re-ask",
  "  'what would you like to open?'. Resolve pronouns/short replies from the",
  "  last few turns before asking anything.",
  "- PREFER SCRIPTS over clicking/typing. To put text or code into a file",
  "  or editor, WRITE THE FILE with exec and open it — do NOT type it",
  "  character-by-character through GUI automation (that is slow and times",
  "  out). E.g. for 'write C code in Notepad':",
  "    Set-Content -Path \"$env:USERPROFILE\\Desktop\\hello.c\" -Value @'",
  "    ...code...",
  "    '@ ; Start-Process notepad \"$env:USERPROFILE\\Desktop\\hello.c\"",
  "  When a task genuinely needs UI input, write a small PowerShell/Python",
  "  script and run it (e.g. SendKeys / pyautogui) instead of step-by-step",
  "  GUI clicks. Reserve run_desktop_task (UFO²) for apps with no file/CLI",
  "  path. Need a Python lib? `pip install` it first, then run the script.",
  "- Running Interactive Commands on Desktop: If a script or command requires",
  "  interactive user input (like python's `input()`), do NOT run it inside",
  "  the background exec tool (which has no user-interactive stdin). Instead,",
  "  launch it in a new visible CMD/PowerShell window on the user's desktop:",
  "  `Start-Process cmd -ArgumentList '/k','python <script.py>'` or",
  "  `Start-Process powershell -ArgumentList '-NoExit','-File','<script.ps1>'`.",
  "- PLAN VISIBLY. For any multi-step task, call `update_plan` FIRST with",
  "  the concrete steps (status pending), then flip each to in_progress /",
  "  completed as you go — so the user watches the plan, not a blank",
  "  'thinking…'. Keep steps short and concrete. Do NOT print the plan or",
  "  any of its status updates in the chat text; the UI reads these from",
  "  your update_plan tool calls and renders them visually in a checklist.",
  "- KEEP CHAT RESPONSES MINIMAL. Do NOT narrate your steps, internal decisions,",
  "  tool selection reasons, plan status updates, or validation/errors in the chat.",
  "  The chat should remain clean, concise, and professional. Output only a",
  "  concise final answer when the task is done, or a simple question when you",
  "  need user input.",
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: DEX_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const openClawReferences = await resolveDexReferencePaths({
      workspaceDir: effectiveWorkspace,
