export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_DEX_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_DEX_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[DEX_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[DEX_INTERNAL_CONTEXT_END]]";

export const DEX_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const DEX_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "OpenClaw runtime context for the immediately preceding user message.";
export const DEX_RUNTIME_EVENT_HEADER = "OpenClaw runtime event.";
export const DEX_RUNTIME_CONTEXT_CUSTOM_TYPE = "openclaw.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["OpenClaw runtime context (internal):", DEX_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === DEX_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === DEX_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === DEX_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${DEX_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${DEX_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${DEX_RUNTIME_EVENT_HEADER}\n${DEX_RUNTIME_CONTEXT_NOTICE}`)
  );
}
...
function isOpenClawRuntimeContextCustomMessage(message: unknown): boolean {
  ...
  return (
    candidate.role === "custom" && candidate.customType === DEX_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}
