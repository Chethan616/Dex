...
const GOOGLE_LIVE_SINGLE_KEY = "DEX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "DEX_LIVE_ANTHROPIC_KEY",
    listVar: "DEX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "DEX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `DEX_LIVE_${base}_KEY`;
