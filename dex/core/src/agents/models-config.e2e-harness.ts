...
export function installModelsConfigTestHooks(opts?: {
  ...
  beforeEach(() => {
    ...
    previousOpenClawAgentDir = process.env.DEX_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousOpenClawAgentDir === undefined) {
      ...
      process.env.DEX_AGENT_DIR = previousOpenClawAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "DEX_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: DexConfig = {
  models: {
    providers: {
      "custom-proxy": {
