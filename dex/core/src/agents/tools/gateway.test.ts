...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.DEX_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.openclaw === undefined) {
      ...
      process.env.DEX_GATEWAY_TOKEN = envSnapshot.openclaw;
    }
  });
  ...
  it("uses DEX_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.DEX_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
