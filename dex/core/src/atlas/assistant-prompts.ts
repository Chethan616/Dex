...
export const ATLAS_ASSISTANT_TIMEOUT_MS = 10_000;
export const ATLAS_ASSISTANT_MAX_TOKENS = 512;

export const ATLAS_ASSISTANT_SYSTEM_PROMPT = [
  "You are Atlas, OpenClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe OpenClaw Atlas command.",
...
export type AtlasAssistantPlan = {
  command: string;
  reply?: string;
  modelLabel?: string;
};

export function buildAtlasAssistantUserPrompt(params: {
  input: string;
...
export function parseAtlasAssistantPlanText(
  rawText: string | undefined,
): AtlasAssistantPlan | null {
