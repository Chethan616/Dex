...
import {
  ...
  planAtlasCommandWithLocalRuntime,
  parseAtlasAssistantPlanText,
} from "./assistant.js";
import type { AtlasOverview } from "./overview.js";

function overview(overrides: Partial<AtlasOverview["tools"]> = {}): AtlasOverview {
  return {
    config: {
...
describe("Atlas assistant", () => {
  ...
  it("rejects non-command output", () => {
    expect(parseAtlasAssistantPlanText("I would edit config directly.")).toBeNull();
    expect(parseAtlasAssistantPlanText('{"reply":"missing command"}')).toBeNull();
  });
  ...
  it("uses Claude CLI first for configless planning", async () => {
    ...
    const result = await planAtlasCommandWithLocalRuntime({
      ...
      deps: {
        runCliAgent,
        runEmbeddedAgent,
        createTempDir: async () => "/tmp/atlas-planner",
        removeTempDir: async () => {},
      },
    });
    if (result === null) {
  ...
  it("selects local planner backends without execution state", () => {
    expect(
      selectAtlasLocalPlannerBackends(
        overview({
          claude: { command: "claude", found: true },
          codex: { command: "codex", found: true },
        }),
    ...
    const [codexAppServer] = selectAtlasLocalPlannerBackends(
      overview({
        codex: { command: "codex", found: true },
      }),
    );
