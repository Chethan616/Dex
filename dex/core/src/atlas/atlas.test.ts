...
describe("runAtlas", () => {
  it("uses the assistant planner only to choose typed operations", async () => {
    ...
    expect(lines.join("\n")).toContain("[atlas] planner: openai/gpt-5.5");
    expect(lines.join("\n")).toContain("[atlas] interpreted: restart gateway");
  ...
  it("starts interactive Atlas in the TUI shell", async () => {
    const { runtime, lines } = createAtlasTestRuntime();
  ...
  it.each([
    ...
    expect(lines.join("\n")).toContain(
      "Atlas needs an interactive TTY. Use --message for one command.",
    );
  });
});
