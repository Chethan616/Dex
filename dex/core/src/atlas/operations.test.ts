...
import { createAtlasTestRuntime } from "./atlas.test-helpers.js";
import { executeAtlasOperation, parseAtlasOperation } from "./operations.js";
...
vi.mock("./overview.js", () => ({
  formatAtlasOverview: () => "Default model: openai/gpt-5.5",
  loadAtlasOverview: vi.fn(async () => ({
    defaultAgentId: "main",
...
describe("parseAtlasOperation", () => {
  ...
  it("parses typed model writes", () => {
    expect(parseAtlasOperation("set default model openai/gpt-5.2")).toEqual({
      kind: "set-default-model",
      model: "openai/gpt-5.2",
    });
    expect(parseAtlasOperation("configure models openai/gpt-5.2")).toEqual({
      kind: "set-default-model",
      model: "openai/gpt-5.2",
    });
  });
  ...
  it("parses verbal agent switching", () => {
    expect(parseAtlasOperation("talk to work agent")).toEqual({
      kind: "open-tui",
      agentId: "work",
    });
  });
  ...
  it("keeps ambiguous model requests read-only", () => {
    expect(parseAtlasOperation("models please")).toEqual({ kind: "models" });
  });

  it("parses gateway lifecycle operations", () => {
    expect(parseAtlasOperation("gateway status")).toEqual({ kind: "gateway-status" });
    expect(parseAtlasOperation("restart gateway")).toEqual({ kind: "gateway-restart" });
    expect(parseAtlasOperation("start gateway")).toEqual({ kind: "gateway-start" });
    expect(parseAtlasOperation("stop gateway")).toEqual({ kind: "gateway-stop" });
  });

  it("parses config and doctor repair operations", () => {
    expect(parseAtlasOperation("validate config")).toEqual({ kind: "config-validate" });
    expect(parseAtlasOperation("config set gateway.port 19001")).toEqual({
      kind: "config-set",
      path: "gateway.port",
      value: "19001",
    });
    expect(parseAtlasOperation("config set-ref gateway.auth.token env GATEWAY_TOKEN")).toEqual(
      {
        kind: "config-set-ref",
    ...
    expect(parseAtlasOperation("doctor fix")).toEqual({ kind: "doctor-fix" });
  });

  it("parses plugin management operations", () => {
    expect(parseAtlasOperation("plugins list")).toEqual({ kind: "plugin-list" });
    expect(parseAtlasOperation("list plugin")).toEqual({ kind: "plugin-list" });
    expect(parseAtlasOperation("plugins search calendar sync")).toEqual({
      kind: "plugin-search",
      query: "calendar sync",
    });
    expect(parseAtlasOperation("install npm plugin @openclaw/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@openclaw/demo",
    });
    expect(parseAtlasOperation("plugin install clawhub:openclaw-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:openclaw-demo",
    });
    expect(parseAtlasOperation("plugin uninstall openclaw-demo")).toEqual({
  ...
  it("parses agent creation requests", () => {
    expect(
      parseAtlasOperation("create agent Work workspace /tmp/work model openai/gpt-5.2"),
    ).toEqual({
      kind: "create-agent",
      agentId: "work",
      workspace: "/tmp/work",
    ...
    expect(parseAtlasOperation("add agent ops")).toEqual({
      kind: "create-agent",
      agentId: "ops",
    });
    expect(parseAtlasOperation("setup workspace /tmp/work model openai/gpt-5.5")).toEqual({
      kind: "setup",
      workspace: "/tmp/work",
      model: "openai/gpt-5.5",
    });
    expect(parseAtlasOperation("setup agent ops")).toEqual({
      kind: "create-agent",
      agentId: "ops",
    });
  });
  ...
  it("requires approval before restarting gateway", async () => {
    const { runtime, lines } = createAtlasTestRuntime();
    const runGatewayRestart = vi.fn(async () => {});

    const result = await executeAtlasOperation({ kind: "gateway-restart" }, runtime, {
  ...
  it("validates missing config without exiting the process", async () => {
    ...
    const result = await executeAtlasOperation({ kind: "config-validate" }, runtime);
  ...
  it("applies config set through typed deps and writes an audit entry", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-config-set-"));
    ...
    expect(lines.join("\n")).toContain("[atlas] done: config.set");
  ...
  it("applies SecretRef config set through typed deps and writes an audit entry", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-config-ref-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);
    const { runtime, lines } = createAtlasTestRuntime();
    const runConfigSet = vi.fn(async () => {});
    ...
    expect(lines.join("\n")).toContain("[atlas] done: config.setRef");
    const auditPath = path.join(tempDir, "audit", "atlas.jsonl");
    const audit = JSON.parse((await fs.readFile(auditPath, "utf8")).trim());
    expectAuditRecord(
      audit,
  ...
  it("runs plugin list and search as read-only operations", async () => {
    ...
    const listResult = await executeAtlasOperation({ kind: "plugin-list" }, runtime, {
      deps: { runPluginsList, runPluginsSearch },
    });
    expect(listResult.applied).toBe(false);
    const searchResult = await executeAtlasOperation(
    ...
    expect(lines.join("\n")).toContain("[atlas] done: plugins.search");
  });

  it("installs plugins only after approval and audits the write", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-plugin-install-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);
    const { runtime, lines } = createAtlasTestRuntime();
    ...
    const plan = await executeAtlasOperation(
    ...
    expect(lines.join("\n")).toContain("[atlas] done: plugin.install");
    const auditPath = path.join(tempDir, "audit", "atlas.jsonl");
    const audit = JSON.parse((await fs.readFile(auditPath, "utf8")).trim());
    expectAuditRecord(
      audit,
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-plugin-uninstall-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);
    const { runtime, lines } = createAtlasTestRuntime();
    ...
    expect(lines.join("\n")).toContain("[atlas] done: plugin.uninstall");
    const auditPath = path.join(tempDir, "audit", "atlas.jsonl");
    const audit = JSON.parse((await fs.readFile(auditPath, "utf8")).trim());
    expectAuditRecord(
      audit,
  ...
  it("runs setup bootstrap only after approval and audits it", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-setup-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);
    ...
    expect(lines.join("\n")).toContain("[atlas] done: atlas.setup");
  ...
  it("runs doctor repairs only after approval and audits them", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-doctor-fix-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);
    const { runtime, lines } = createAtlasTestRuntime();
    const runDoctor = vi.fn(async () => {});

    const plan = await executeAtlasOperation({ kind: "doctor-fix" }, runtime, {
      deps: { runDoctor },
    });
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
    ...
    const result = await executeAtlasOperation({ kind: "doctor-fix" }, runtime, {
      approved: true,
      deps: { runDoctor },
      auditDetails: { rescue: true },
    });
    ...
    expect(lines.join("\n")).toContain("[atlas] done: doctor.fix");
    const auditPath = path.join(tempDir, "audit", "atlas.jsonl");
  ...
  it("returns from the agent TUI back to Atlas", async () => {
    const { runtime, lines } = createAtlasTestRuntime();
    const runTui = vi.fn(async () => ({
      exitReason: "return-to-atlas" as const,
    ...
    expect(lines.join("\n")).toContain(
      "[atlas] returned from agent with request: restart gateway",
    );
  });
});
