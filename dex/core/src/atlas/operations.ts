...
import { appendAtlasAuditEntry, resolveAtlasAuditPath } from "./audit.js";
...
type AtlasOverviewLoader = () => Promise<AtlasOverview>;
type AtlasOverviewFormatter = (overview: AtlasOverview) => string;
...
export type AtlasOperationResult = {
  applied: boolean;
...
export type AtlasCommandDeps = {
  formatOverview?: AtlasOverviewFormatter;
  loadOverview?: AtlasOverviewLoader;
  runAgentsAdd?: (
    opts: {
      name?: string;
      workspace?: string;
...
export function parseAtlasOperation(input: string): AtlasOperation {
  ...
  if (lower === "quit" || lower === "exit") {
    return { kind: "none", message: "Atlas retracts into shell. Bye." };
  }
  return {
    kind: "none",
    message:
...
function validateAtlasPluginInstallSpec(spec: string): string | null {
  const trimmed = spec.trim();
  if (!trimmed) {
    return "Plugin install spec is required.";
  }
  if (/\s/.test(trimmed)) {
    return "Atlas plugin install accepts one npm or ClawHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    return "Atlas plugin install accepts npm or ClawHub package specs only.";
  }
  return null;
}

export function isPersistentAtlasOperation(operation: AtlasOperation): boolean {
  return (
...
export function describeAtlasPersistentOperation(operation: AtlasOperation): string {
...
export function formatAtlasPersistentPlan(operation: AtlasOperation): string {
  return `Plan: ${describeAtlasPersistentOperation(operation)}. Say yes to apply.`;
}
...
function formatSetupPlanDescription(
  operation: Extract<AtlasOperation, { kind: "setup" }>,
): string {
...
function logQueued(runtime: RuntimeEnv, operation: string): void {
  runtime.log(`[atlas] queued: ${operation}`);
  runtime.log(`[atlas] running: ${operation}`);
}

function formatGatewayStatusLine(overview: AtlasOverview): string {
  return [
...
async function loadOverviewForOperation(
  deps: AtlasCommandDeps | undefined,
): Promise<AtlasOverview> {
  if (deps?.loadOverview) {
    return await deps.loadOverview();
  }
  const { loadAtlasOverview } = await loadOverviewModule();
...
async function formatOverviewForOperation(
  ...
  const { formatAtlasOverview } = await loadOverviewModule();
  return formatAtlasOverview(overview);
}
...
export async function executeAtlasOperation(
  ...
  if (operation.kind === "plugin-list") {
    ...
    runtime.log("[atlas] done: plugins.list");
    return { applied: false };
  }
  if (operation.kind === "plugin-search") {
    ...
    runtime.log("[atlas] done: plugins.search");
    return { applied: false };
  }
  if (operation.kind === "audit") {
    runtime.log(`Audit log: ${resolveAtlasAuditPath()}`);
  ...
  if (operation.kind === "setup") {
    ...
    if (!opts.approved) {
      const message = [
        formatAtlasPersistentPlan(operation),
        setupModel.model
    ...
    runtime.log("[atlas] done: atlas.setup");
    return { applied: true };
  }
  if (operation.kind === "config-set") {
    if (!opts.approved) {
      const message = formatAtlasPersistentPlan(operation);
      runtime.log(message);
      return { applied: false, message };
    }
    ...
    runtime.log("[atlas] done: config.set");
    return { applied: true };
  }
  if (operation.kind === "config-set-ref") {
    ...
    runtime.log("[atlas] done: config.setRef");
    return { applied: true };
  }
  if (operation.kind === "plugin-install") {
    ...
    const validationError = validateAtlasPluginInstallSpec(operation.spec);
    if (validationError) {
    ...
    runtime.log("[atlas] done: plugin.install");
  ...
  if (operation.kind === "plugin-uninstall") {
    ...
    runtime.log("[atlas] done: plugin.uninstall");
  ...
  if (operation.kind === "create-agent") {
    ...
    runtime.log("[atlas] done: agents.create");
    return { applied: true };
  }
  ...
  if (operation.kind === "doctor-fix") {
    ...
    runtime.log("[atlas] done: doctor.fix");
    return { applied: true };
  }
  if (operation.kind === "status") {
    ...
    runtime.log("[atlas] done: status.check");
    return { applied: false };
  }
  if (operation.kind === "health") {
    ...
    runtime.log("[atlas] done: health.check");
    return { applied: false };
  }
  ...
  if (operation.kind === "gateway-start") {
    ...
    runtime.log("[atlas] done: gateway.start");
    return { applied: true };
  }
  if (operation.kind === "gateway-stop") {
    ...
    runtime.log("[atlas] done: gateway.stop");
    return { applied: true };
  }
  if (operation.kind === "gateway-restart") {
    ...
    runtime.log("[atlas] done: gateway.restart");
    return { applied: true };
  }
  if (operation.kind === "open-tui") {
    ...
    if (result?.exitReason === "return-to-atlas") {
      runtime.log(
        result.atlasMessage
          ? `[atlas] returned from agent with request: ${result.atlasMessage}`
          : "[atlas] returned from agent",
      );
      return {
        applied: false,
        nextInput: result.atlasMessage,
      };
    }
  ...
  if (operation.kind === "set-default-model") {
    ...
    runtime.log("[atlas] done: config.setDefaultModel");
    return { applied: true };
  }
  return { applied: false };
}
