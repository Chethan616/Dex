...
import {
  formatAtlasOverview,
  formatAtlasStartupMessage,
  loadAtlasOverview,
} from "./overview.js";

describe("loadAtlasOverview", () => {
  it("summarizes config, agents, model, tools, and gateway", async () => {
    ...
    const overview = await loadAtlasOverview({
      env: { DEX_TEST_FAST: "1" },
      deps: {
    ...
    expect(formatAtlasOverview(overview)).toContain(
      'Next: run "gateway status" or "restart gateway"',
    );
    const startup = formatAtlasStartupMessage(overview);
    expect(startup).toContain("## Hi, I'm Atlas.");
