...
type AtlasOverviewDependencies = {
...
function buildAgentSummaries(cfg: DexConfig): AtlasAgentSummary[] {
  ...
  const summaries: AtlasAgentSummary[] = [];
  for (const entry of entries) {
    ...
    seen.add(id);
    const summary: AtlasAgentSummary = {
      id,
...
export async function loadAtlasOverview(
  opts: { env?: NodeJS.ProcessEnv; deps?: AtlasOverviewDependencies } = {},
...
export function formatAtlasOverview(overview: AtlasOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Atlas online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `Next: ${recommendAtlasNextStep(overview)}`,
    ...issueLines,
  ]
...
function recommendAtlasNextStep(overview: AtlasOverview): string {
...
function formatStartupConfigStatus(overview: AtlasOverview): string {
  if (!overview.config.exists) {
    return "missing";
  }
...
function formatStartupUse(overview: AtlasOverview): string {
  if (overview.defaultModel) {
...
function formatStartupGatewayStatus(overview: AtlasOverview): string {
  if (overview.gateway.reachable) {
...
function formatStartupAction(overview: AtlasOverview): string {
  if (!overview.config.exists) {
...
export function formatAtlasStartupMessage(overview: AtlasOverview): string {
