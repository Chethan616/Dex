...
import { extractAtlasRescueMessage, runAtlasRescueMessage } from "./rescue-message.js";
...
function commandContext(overrides: Partial<CommandContext> = {}): CommandContext {
  return {
    ...
    senderId: "user:owner",
    rawBodyNormalized: "/atlas models",
    commandBodyNormalized: "/atlas models",
    from: "user:owner",
    to: "account:default",
    ...overrides,
  };
...
async function runRescue(
  ...
  cfg: DexConfig,
  ctx = commandContext(),
  deps?: Parameters<typeof runAtlasRescueMessage>[0]["deps"],
) {
...
describe("Atlas rescue message", () => {
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atlas-rescue-"));
  });
  ...
  it("recognizes the Atlas rescue command", () => {
    expect(extractAtlasRescueMessage("/atlas status")).toBe("status");
    expect(extractAtlasRescueMessage("/atlas")).toBe("");
    expect(extractAtlasRescueMessage("/status")).toBeNull();
  });

  it("denies rescue when sandboxing is active", async () => {
    await expect(
      runRescue("/atlas status", {
  ...
  it("refuses TUI handoff from remote rescue", async () => {
    const cfg: DexConfig = { atlas: { rescue: { enabled: true } } };
    const deps = {
      runTui: vi.fn(async () => {
    ...
    await expect(
      runRescue("/atlas talk to agent", cfg, commandContext(), deps),
    ).resolves.toContain("cannot open the local TUI");
    await expect(runRescue("/atlas chat", cfg, commandContext(), deps)).resolves.toContain(
  ...
  it("refuses plugin install from remote rescue", async () => {
    ...
    await expect(
      runRescue("/atlas plugin install clawhub:openclaw-demo", cfg, commandContext(), deps),
  ...
  it("allows plugin list and search from remote rescue", async () => {
    ...
    await expect(
      runRescue("/atlas plugins list", cfg, commandContext(), deps),
    ).resolves.toContain("plugin rows");
    await expect(
      runRescue("/atlas plugins search calendar", cfg, commandContext(), deps),
  ...
  it("queues and applies persistent writes through conversational approval", async () => {
    ...
    const cfg: DexConfig = { atlas: { rescue: { enabled: true } } };
    await expect(runRescue("/atlas set default model openai/gpt-5.2", cfg)).resolves.toContain(
      "Reply /atlas yes to apply",
    );
    await expect(runRescue("/atlas yes", cfg)).resolves.toContain(
  ...
  it("queues and applies gateway restart through conversational approval", async () => {
    ...
    await expect(
      runRescue("/atlas restart gateway", cfg, commandContext(), deps),
    ).resolves.toBe("Plan: restart the Gateway. Reply /atlas yes to apply.");
    await expect(runRescue("/atlas yes", cfg, commandContext(), deps)).resolves.toContain(
      "[atlas] done: gateway.restart",
    );
  ...
  it("does not queue persistent rescue approval when expiry would exceed the Date range", async () => {
    ...
    try {
      ...
      await expect(
        runRescue("/atlas restart gateway", cfg, commandContext()),
      ).resolves.toContain("expiry clock is invalid");

      await expect(fs.readdir(path.join(tempDir, "atlas", "rescue-pending"))).rejects.toThrow(
        /ENOENT/,
      );
    } finally {
      vi.useRealTimers();
  ...
  it("rejects pending rescue approvals with invalid persisted expiry", async () => {
    ...
    const pendingDir = path.join(tempDir, "atlas", "rescue-pending");
    ...
    await expect(runRescue("/atlas yes", cfg, commandContext(), deps)).resolves.toBe(
      "No pending Atlas rescue change is waiting for approval.",
    );
  ...
  it("queues and applies agent creation through conversational approval", async () => {
    ...
    await expect(
      runRescue("/atlas create agent work workspace /tmp/work", cfg, commandContext(), deps),
    ).resolves.toBe(
      "Plan: create agent work with workspace /tmp/work. Reply /atlas yes to apply.",
    );
    await expect(runRescue("/atlas yes", cfg, commandContext(), deps)).resolves.toContain(
      "[atlas] done: agents.create",
    );
