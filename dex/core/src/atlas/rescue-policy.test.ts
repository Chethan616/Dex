...
function decide(cfg: DexConfig, overrides = {}) {
  return resolveAtlasRescuePolicy({
    cfg,
    senderIsOwner: true,
...
describe("resolveAtlasRescuePolicy", () => {
  ...
  it("allows explicit group rescue when ownerDmOnly is disabled", () => {
    expect(
      decide({ atlas: { rescue: { ownerDmOnly: false } } }, { isDirectMessage: false })
        .allowed,
    ).toBe(true);
  });
});
