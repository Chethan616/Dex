...
export function resolveAtlasRescuePolicy(
  input: AtlasRescuePolicyInput,
): AtlasRescueDecision {
  const rescue = input.cfg.atlas?.rescue;
  ...
  if (!enabled) {
    return {
      ...
      reason: "disabled",
      message:
        "Atlas rescue is disabled. Set atlas.rescue.enabled=true or use YOLO host posture with sandboxing off.",
    };
  }
  if (sandboxActive) {
    return {
      ...
      message:
        "Atlas rescue is blocked because OpenClaw sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  if (configuredEnabled === "auto" && !yolo) {
    return {
      ...
      reason: "not-yolo",
      message:
        "Atlas rescue auto-mode only opens in YOLO host posture: tools.exec.security=full, tools.exec.ask=off, and sandboxing off.",
    };
  }
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Atlas rescue only accepts commands from an OpenClaw owner.",
    };
  }
  if (ownerDmOnly && !input.isDirectMessage) {
    return {
      ...
      message: "Atlas rescue is restricted to owner DMs by default.",
    };
  }
  return {
    allowed: true,
