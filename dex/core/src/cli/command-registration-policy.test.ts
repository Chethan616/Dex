...
describe("command-registration-policy", () => {
  ...
  it("matches lazy subcommand registration policy", () => {
    expect(shouldEagerRegisterSubcommands({ DEX_DISABLE_LAZY_SUBCOMMANDS: "1" })).toBe(true);
    expect(shouldEagerRegisterSubcommands({ DEX_DISABLE_LAZY_SUBCOMMANDS: "0" })).toBe(false);
    ...
    expect(
      shouldRegisterPrimarySubcommandOnly(["node", "openclaw", "acp"], {
        DEX_DISABLE_LAZY_SUBCOMMANDS: "1",
      }),
    ).toBe(false);
  });
});
