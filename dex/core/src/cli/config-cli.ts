...
import { isRecord as isPlainRecord } from "@dexagent/normalization-core/record-coerce";
...
const CONFIG_SET_EXAMPLE_VALUE = formatCliCommand(
  "dex config set gateway.port 19001 --strict-json",
);
const CONFIG_SET_EXAMPLE_REF = formatCliCommand(
  "dex config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
);
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "dex config set secrets.providers.vault --provider-source file --provider-path /etc/openclaw/secrets.json --provider-mode json",
);
const CONFIG_SET_EXAMPLE_BATCH = formatCliCommand(
  "dex config set --batch-file ./config-set.batch.json --dry-run",
);
const CONFIG_PATCH_EXAMPLE_FILE = formatCliCommand(
  "dex config patch --file ./openclaw.patch.json5 --dry-run",
);
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("dex config patch --stdin");
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("dex plugins install <spec>")}`,
    `  ${formatCliCommand("dex plugins update <plugin-id>")}`,
    `  ${formatCliCommand("dex plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("dex config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("dex config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
  ...
  try {
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(`Inspect with ${formatCliCommand("dex config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
