...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to DEX_CONTAINER", () => {
    expect(
...
describe("maybeRunCliInContainer", () => {
  ...
  it("uses DEX_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "openclaw", "status"], {
        env: { DEX_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("passes the proxy URL env fallback into the child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "openclaw", "status"], {
      env: {
        DEX_CONTAINER: "demo",
        DEX_PROXY_URL: " http://proxy.internal:3128 ",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("redacts proxy URL credentials and URL suffixes before rejecting loopback container proxy forwarding", () => {
    ...
    expect(message).toContain("DEX_PROXY_URL=http://redacted:redacted@127.0.0.1:3128/");
  ...
  it("allows explicitly overridden loopback proxy URL forwarding into a child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "openclaw", "status"], {
      env: {
        ...
        DEX_CONTAINER_ALLOW_LOOPBACK_PROXY_URL: "1",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expect(podmanCall[1]).toContain("DEX_PROXY_URL=http://127.0.0.1:3128");
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "openclaw", DEX_CONTAINER: "" },
      },
    );
  });
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", DEX_CONTAINER: "" },
      },
    );
  ...
  it("prefers --container over DEX_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "openclaw", "--container", "flag-demo", "health"], {
        env: { DEX_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "openclaw", "--container", "demo", "status"], {
        env: { DEX_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
