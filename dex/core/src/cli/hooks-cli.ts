...
function buildHooksReport(config: DexConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("dex hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("dex hooks list")}\` to see available hooks.`;
  }
...
export async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as DexConfig;
...
export function registerHooksCli(program: Command): void {
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `dex plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`dex hooks install` is deprecated; use `dex plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `dex plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`dex hooks update` is deprecated; use `dex plugins update`."),
      );
