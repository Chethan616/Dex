    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "dex node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["dex node status", "Check node host service status."],
          ["dex node install", "Install the node host service."],
          ["dex node start", "Start the installed node host service."],
          ["dex node restart", "Restart the installed node host service."],
