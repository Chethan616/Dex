...
describe("applyCliProfileEnv", () => {
  it("fills env defaults for dev profile", () => {
    ...
    expect(env.DEX_PROFILE).toBe("dev");
    expect(env.DEX_STATE_DIR).toBe(expectedStateDir);
    expect(env.DEX_CONFIG_PATH).toBe(path.join(expectedStateDir, "openclaw.json"));
    expect(env.DEX_GATEWAY_PORT).toBe("19001");
  });

  it("does not override explicit env values", () => {
    const env: Record<string, string | undefined> = {
      DEX_PROFILE: "prod",
      DEX_STATE_DIR: "/custom",
      DEX_GATEWAY_PORT: "19099",
    };
    applyCliProfileEnv({
      profile: "dev",
      env,
    ...
    expect(env.DEX_PROFILE).toBe("dev");
    expect(env.DEX_STATE_DIR).toBe("/custom");
    expect(env.DEX_GATEWAY_PORT).toBe("19099");
    expect(env.DEX_CONFIG_PATH).toBe(path.join("/custom", "openclaw.json"));
  });

  it("uses DEX_HOME when deriving profile state dir", () => {
    const env: Record<string, string | undefined> = {
    ...
    expect(env.DEX_STATE_DIR).toBe(path.join(resolvedHome, ".openclaw-work"));
...
describe("formatCliCommand", () => {
  ...
  it("handles command with no args after openclaw", () => {
    expect(formatCliCommand("openclaw", { DEX_PROFILE: "test" })).toBe(
