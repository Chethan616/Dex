...
export function registerBackupCommand(program: Command) {
  const backup = program
    .command("backup")
    .description("Create and verify local backup archives for Dex state")
    .addHelpText(
      "after",
      () =>
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["dex backup create", "Create a timestamped backup in the current directory."],
          [
            "dex backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "dex backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "dex backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "dex backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["dex backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "dex backup verify ./2026-03-09T08-00-00.000+08-00-openclaw-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "dex backup verify ~/Backups/latest.tar.gz --json",
