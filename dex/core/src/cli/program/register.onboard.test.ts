...
describe("registerOnboardCommand", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.runAtlas.mockResolvedValue(undefined);
  ...
  it("defaults installDaemon to undefined when no daemon flags are provided", async () => {
    ...
    expect(mocks.runAtlas).not.toHaveBeenCalled();
  });
  ...
  it("forwards --gateway-token-ref-env", async () => {
    await runCli(["onboard", "--gateway-token-ref-env", "DEX_GATEWAY_TOKEN"]);
    expect(setupWizardOptions().gatewayTokenRefEnv).toBe("DEX_GATEWAY_TOKEN");
  });
  ...
  it("routes --modern to Atlas", async () => {
    await runCli(["onboard", "--modern", "--json"]);

    expect(setupWizardCommandMock).not.toHaveBeenCalled();
    expect(mocks.runAtlas).toHaveBeenCalledWith({
      message: undefined,
      yes: false,
      json: true,
      interactive: true,
