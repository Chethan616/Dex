...
function readDevicePairPublicUrlFromConfig(cfg: DexConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.DEX_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: DexConfig): Promise<void> {
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        lines.push(
          ...
          `  ${theme.command("dex devices list")}`,
          `  ${theme.command("dex devices approve <requestId>")}`,
        );
