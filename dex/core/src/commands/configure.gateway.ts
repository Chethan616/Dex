    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "DEX_GATEWAY_TOKEN",
          placeholder: "DEX_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like DEX_GATEWAY_TOKEN.";
            }
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        ...
        "Docs: https://docs.dex.run/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
