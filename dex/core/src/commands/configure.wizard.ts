...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.DEX_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.DEX_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.dex.run/gateway/health",
        "https://docs.dex.run/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<DexConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Dex update wizard" : "Dex configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: DexConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("dex doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.DEX_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.DEX_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.dex.run/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    const newPassword =
      process.env.DEX_GATEWAY_PASSWORD ??
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.dex.run/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
