...
const PRESERVED_DEX_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "DEX_CLI_CONTAINER_BYPASS",
  "DEX_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("OPENCLAW_") && !PRESERVED_DEX_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[DEX_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${DEX_WRAPPER_ENV_KEY} because it points to the Windows task script; using the OpenClaw gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  ...
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [DEX_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, DEX_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.DEX_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("dex gateway install")}\` after fixing the error.`;
}
