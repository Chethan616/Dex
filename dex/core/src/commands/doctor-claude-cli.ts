...
function usesClaudeCliModelSelection(cfg: DexConfig): boolean {
...
function resolveClaudeCliCommand(cfg: DexConfig): string {
...
function resolveClaudeCliAgentIds(cfg: DexConfig): string[] {
  ...
  if (credential) {
    ...
    fixHints.push(
      `- Fix: run ${formatCliCommand("claude auth login")}, then ${formatCliCommand(
        "dex models auth login --provider anthropic --method cli --set-default",
      )}.`,
    );
  }
