...
function renderBlockingSystemGatewayServices(services: ExtraGatewayService[]): string {
  return [
    ...
    "Run `dex gateway status --deep` or `dex doctor --deep` to inspect duplicate services.",
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (!loaded) {
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (!install) {
        note(
          `Run ${formatCliCommand("dex gateway install")} when you want to install the gateway service.`,
          "Gateway",
        );
      }
      if (install) {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.DEX_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("dex gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
