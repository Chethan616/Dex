...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: DexConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: DexConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("dex doctor --fix")} or ${formatCliCommand("dex memory status --fix")}`
    : `Verify: ${formatCliCommand("dex memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: DexConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: DexConfig): boolean {
 ...
 * Runs as part of `dex doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("dex config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("dex config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("dex memory status --deep")}`,
        ]
          .filter(Boolean)
    ...
    note(
      [
        ...
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`dex config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("dex config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("dex memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("dex config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("dex memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `dex doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("dex configure --section model")}`,
      `- To disable: ${formatCliCommand("dex config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("dex memory status --deep")}`,
    ].join("\n"),
    "Memory search",
