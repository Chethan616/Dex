...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("dex tasks maintenance --apply");
  ...
  it("prints openclaw-only verification hints when recent sessions are missing transcripts", async () => {
    ...
    expect(text).toMatch(/dex sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/dex sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /dex sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
