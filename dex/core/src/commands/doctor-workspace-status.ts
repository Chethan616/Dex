...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("dex tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("dex tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: DexConfig) {
