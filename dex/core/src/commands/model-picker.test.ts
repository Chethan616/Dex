import type { NormalizedModelCatalogRow } from "@dexagent/model-catalog-core/model-catalog-types";
...
const hasUsableCustomProviderApiKey = vi.hoisted(() =>
  vi.fn<(_cfg?: DexConfig, _provider?: string, _env?: NodeJS.ProcessEnv) => boolean>(
    () => false,
  ),
);
...
const createProviderAuthChecker = vi.hoisted(() =>
  vi.fn(
    (params: { cfg?: DexConfig; workspaceDir?: string; env?: NodeJS.ProcessEnv }) =>
...
describe("promptDefaultModel", () => {
  it("adds runtime-route hints for canonical OpenAI models", async () => {
    ...
    await promptDefaultModel({
      config: { agents: { defaults: {} } } as DexConfig,
      prompter,
      allowKeep: false,
      includeManual: false,
  ...
  it("uses the configured default agent dir for provider-scoped catalog auth", async () => {
    ...
    const env = {
      ...process.env,
      DEX_STATE_DIR: "/tmp/openclaw-picker-state",
    };
    const config = {
      agents: {
  ...
  it("supports configuring vLLM during setup", async () => {
    ...
    const prompter = makePrompter({ select });
    const config = { agents: { defaults: {} } } as DexConfig;
