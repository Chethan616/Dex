...
export function summarizeExistingConfig(config: DexConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: DexConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  // Sand-framed Dex banner -- mirrors the CLI banner art in cli/banner.ts
  // (Phase B brand swap). One banner, one identity. The upstream lobster
  // block-letter art is gone.
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.dex.run/gateway/remote",
    "https://docs.dex.run/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
