...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("does not fall back to stale DEX_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.DEX_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.DEX_GATEWAY_TOKEN = "stale-env-token";
    process.env.DEX_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
