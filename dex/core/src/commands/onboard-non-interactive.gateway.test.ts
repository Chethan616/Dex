...
const testConfigStore = new Map<string, DexConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.DEX_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.DEX_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("DEX_STATE_DIR must be set before config IO in this test");
  }
...
function readTestConfig<T = DexConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: DexConfig) => cfg.gateway?.port ?? 18789,
}));
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `dex gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `dex gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.DEX_STATE_DIR = stateDir;
      process.env.DEX_CONFIG_PATH = path.join(stateDir, "openclaw.json");
