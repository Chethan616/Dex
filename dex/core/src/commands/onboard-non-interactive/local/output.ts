...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
      return "Fix: run `dex doctor --fix`.";
    case "module-missing":
      return "Fix: run `dex doctor --fix`.";
    case "service-missing":
      return "Fix: run `dex gateway install --force`.";
    case "service-stopped":
      return "Fix: run `dex gateway restart`.";
    case "startup-blocked":
      return "Fix: run `dex gateway status --deep`.";
    case "not-listening":
      return "Fix: start `dex gateway run`, or run `dex gateway restart` for a managed gateway.";
    default:
      return undefined;
  }
}
