...
export async function setupSkills(
  ...
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        ...
        `To inspect available skills, run: ${formatCliCommand("dex skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("dex skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("dex doctor")}\` to review skills + requirements.`,
      );
