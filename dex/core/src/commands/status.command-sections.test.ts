...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:dex security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:dex security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:dex status --all",
      "  Need to debug live? cmd:dex logs --follow",
      "  Fix reachability first: cmd:dex gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:dex devices approve req-123)",
      "muted(Fallback: cmd:dex devices approve --latest)",
      "muted(Inspect: cmd:dex devices list)",
    ]);
  });
