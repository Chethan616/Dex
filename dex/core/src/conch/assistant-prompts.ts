...
export const CONCH_ASSISTANT_TIMEOUT_MS = 10_000;
export const CONCH_ASSISTANT_MAX_TOKENS = 512;

export const CONCH_ASSISTANT_SYSTEM_PROMPT = [
  "You are Conch, OpenClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe OpenClaw Conch command.",
...
export type ConchAssistantPlan = {
  command: string;
  reply?: string;
  modelLabel?: string;
};

export function buildConchAssistantUserPrompt(params: {
  input: string;
...
export function parseConchAssistantPlanText(
  rawText: string | undefined,
): ConchAssistantPlan | null {
