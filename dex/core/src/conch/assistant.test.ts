...
import {
  ...
  planConchCommandWithLocalRuntime,
  parseConchAssistantPlanText,
} from "./assistant.js";
import type { ConchOverview } from "./overview.js";

function overview(overrides: Partial<ConchOverview["tools"]> = {}): ConchOverview {
  return {
    config: {
...
describe("Conch assistant", () => {
  ...
  it("rejects non-command output", () => {
    expect(parseConchAssistantPlanText("I would edit config directly.")).toBeNull();
    expect(parseConchAssistantPlanText('{"reply":"missing command"}')).toBeNull();
  });
  ...
  it("uses Claude CLI first for configless planning", async () => {
    ...
    const result = await planConchCommandWithLocalRuntime({
      ...
      deps: {
        runCliAgent,
        runEmbeddedAgent,
        createTempDir: async () => "/tmp/conch-planner",
        removeTempDir: async () => {},
      },
    });
    if (result === null) {
  ...
  it("selects local planner backends without execution state", () => {
    expect(
      selectConchLocalPlannerBackends(
        overview({
          claude: { command: "claude", found: true },
          codex: { command: "codex", found: true },
        }),
    ...
    const [codexAppServer] = selectConchLocalPlannerBackends(
      overview({
        codex: { command: "codex", found: true },
      }),
    );
