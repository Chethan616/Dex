...
describe("Conch audit log", () => {
  ...
  it("writes jsonl records under the OpenClaw audit dir", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "conch-audit-"));
    vi.stubEnv("DEX_STATE_DIR", tempDir);

    const auditPath = await appendConchAuditEntry({
    ...
    expect(auditPath).toBe(resolveConchAuditPath());
