...
describe("runConch", () => {
  it("uses the assistant planner only to choose typed operations", async () => {
    ...
    expect(lines.join("\n")).toContain("[conch] planner: openai/gpt-5.5");
    expect(lines.join("\n")).toContain("[conch] interpreted: restart gateway");
  ...
  it("starts interactive Conch in the TUI shell", async () => {
    const { runtime, lines } = createConchTestRuntime();
  ...
  it.each([
    ...
    expect(lines.join("\n")).toContain(
      "Conch needs an interactive TTY. Use --message for one command.",
    );
  });
});
