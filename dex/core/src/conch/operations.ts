...
import { appendConchAuditEntry, resolveConchAuditPath } from "./audit.js";
...
type ConchOverviewLoader = () => Promise<ConchOverview>;
type ConchOverviewFormatter = (overview: ConchOverview) => string;
...
export type ConchOperationResult = {
  applied: boolean;
...
export type ConchCommandDeps = {
  formatOverview?: ConchOverviewFormatter;
  loadOverview?: ConchOverviewLoader;
  runAgentsAdd?: (
    opts: {
      name?: string;
      workspace?: string;
...
export function parseConchOperation(input: string): ConchOperation {
  ...
  if (lower === "quit" || lower === "exit") {
    return { kind: "none", message: "Conch retracts into shell. Bye." };
  }
  return {
    kind: "none",
    message:
...
function validateConchPluginInstallSpec(spec: string): string | null {
  const trimmed = spec.trim();
  if (!trimmed) {
    return "Plugin install spec is required.";
  }
  if (/\s/.test(trimmed)) {
    return "Conch plugin install accepts one npm or ClawHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    return "Conch plugin install accepts npm or ClawHub package specs only.";
  }
  return null;
}

export function isPersistentConchOperation(operation: ConchOperation): boolean {
  return (
...
export function describeConchPersistentOperation(operation: ConchOperation): string {
...
export function formatConchPersistentPlan(operation: ConchOperation): string {
  return `Plan: ${describeConchPersistentOperation(operation)}. Say yes to apply.`;
}
...
function formatSetupPlanDescription(
  operation: Extract<ConchOperation, { kind: "setup" }>,
): string {
...
function logQueued(runtime: RuntimeEnv, operation: string): void {
  runtime.log(`[conch] queued: ${operation}`);
  runtime.log(`[conch] running: ${operation}`);
}

function formatGatewayStatusLine(overview: ConchOverview): string {
  return [
...
async function loadOverviewForOperation(
  deps: ConchCommandDeps | undefined,
): Promise<ConchOverview> {
  if (deps?.loadOverview) {
    return await deps.loadOverview();
  }
  const { loadConchOverview } = await loadOverviewModule();
...
async function formatOverviewForOperation(
  ...
  const { formatConchOverview } = await loadOverviewModule();
  return formatConchOverview(overview);
}
...
export async function executeConchOperation(
  ...
  if (operation.kind === "plugin-list") {
    ...
    runtime.log("[conch] done: plugins.list");
    return { applied: false };
  }
  if (operation.kind === "plugin-search") {
    ...
    runtime.log("[conch] done: plugins.search");
    return { applied: false };
  }
  if (operation.kind === "audit") {
    runtime.log(`Audit log: ${resolveConchAuditPath()}`);
  ...
  if (operation.kind === "setup") {
    ...
    if (!opts.approved) {
      const message = [
        formatConchPersistentPlan(operation),
        setupModel.model
    ...
    runtime.log("[conch] done: conch.setup");
    return { applied: true };
  }
  if (operation.kind === "config-set") {
    if (!opts.approved) {
      const message = formatConchPersistentPlan(operation);
      runtime.log(message);
      return { applied: false, message };
    }
    ...
    runtime.log("[conch] done: config.set");
    return { applied: true };
  }
  if (operation.kind === "config-set-ref") {
    ...
    runtime.log("[conch] done: config.setRef");
    return { applied: true };
  }
  if (operation.kind === "plugin-install") {
    ...
    const validationError = validateConchPluginInstallSpec(operation.spec);
    if (validationError) {
    ...
    runtime.log("[conch] done: plugin.install");
  ...
  if (operation.kind === "plugin-uninstall") {
    ...
    runtime.log("[conch] done: plugin.uninstall");
  ...
  if (operation.kind === "create-agent") {
    ...
    runtime.log("[conch] done: agents.create");
    return { applied: true };
  }
  ...
  if (operation.kind === "doctor-fix") {
    ...
    runtime.log("[conch] done: doctor.fix");
    return { applied: true };
  }
  if (operation.kind === "status") {
    ...
    runtime.log("[conch] done: status.check");
    return { applied: false };
  }
  if (operation.kind === "health") {
    ...
    runtime.log("[conch] done: health.check");
    return { applied: false };
  }
  ...
  if (operation.kind === "gateway-start") {
    ...
    runtime.log("[conch] done: gateway.start");
    return { applied: true };
  }
  if (operation.kind === "gateway-stop") {
    ...
    runtime.log("[conch] done: gateway.stop");
    return { applied: true };
  }
  if (operation.kind === "gateway-restart") {
    ...
    runtime.log("[conch] done: gateway.restart");
    return { applied: true };
  }
  if (operation.kind === "open-tui") {
    ...
    if (result?.exitReason === "return-to-conch") {
      runtime.log(
        result.conchMessage
          ? `[conch] returned from agent with request: ${result.conchMessage}`
          : "[conch] returned from agent",
      );
      return {
        applied: false,
        nextInput: result.conchMessage,
      };
    }
  ...
  if (operation.kind === "set-default-model") {
    ...
    runtime.log("[conch] done: config.setDefaultModel");
    return { applied: true };
  }
  return { applied: false };
}
