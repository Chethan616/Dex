...
import {
  formatConchOverview,
  formatConchStartupMessage,
  loadConchOverview,
} from "./overview.js";

describe("loadConchOverview", () => {
  it("summarizes config, agents, model, tools, and gateway", async () => {
    ...
    const overview = await loadConchOverview({
      env: { DEX_TEST_FAST: "1" },
      deps: {
    ...
    expect(formatConchOverview(overview)).toContain(
      'Next: run "gateway status" or "restart gateway"',
    );
    const startup = formatConchStartupMessage(overview);
    expect(startup).toContain("## Hi, I'm Conch.");
