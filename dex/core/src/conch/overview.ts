...
type DexReferencePaths = Awaited<ReturnType<typeof resolveDexReferencePaths>>;
...
type ConchOverviewDependencies = {
  ...
  resolveDexReferencePaths?: typeof resolveDexReferencePaths;
};
...
function buildAgentSummaries(cfg: DexConfig): ConchAgentSummary[] {
  ...
  const summaries: ConchAgentSummary[] = [];
  for (const entry of entries) {
    ...
    seen.add(id);
    const summary: ConchAgentSummary = {
      id,
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): DexReferencePaths | undefined {
  if (env.DEX_TEST_FAST !== "1") {
    return undefined;
  }
...
export async function loadConchOverview(
  opts: { env?: NodeJS.ProcessEnv; deps?: ConchOverviewDependencies } = {},
  ...
  const resolveReferences = deps.resolveDexReferencePaths ?? resolveDexReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: DEX_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: DEX_SOURCE_URL,
    },
  };
}
...
export function formatConchOverview(overview: ConchOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Conch online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `Next: ${recommendConchNextStep(overview)}`,
    ...issueLines,
  ]
...
function recommendConchNextStep(overview: ConchOverview): string {
...
function formatStartupConfigStatus(overview: ConchOverview): string {
  if (!overview.config.exists) {
    return "missing";
  }
...
function formatStartupUse(overview: ConchOverview): string {
  if (overview.defaultModel) {
...
function formatStartupGatewayStatus(overview: ConchOverview): string {
  if (overview.gateway.reachable) {
...
function formatStartupAction(overview: ConchOverview): string {
  if (!overview.config.exists) {
...
export function formatConchStartupMessage(overview: ConchOverview): string {
