...
import { extractConchRescueMessage, runConchRescueMessage } from "./rescue-message.js";
...
function commandContext(overrides: Partial<CommandContext> = {}): CommandContext {
  return {
    ...
    senderId: "user:owner",
    rawBodyNormalized: "/conch models",
    commandBodyNormalized: "/conch models",
    from: "user:owner",
    to: "account:default",
    ...overrides,
  };
...
async function runRescue(
  ...
  cfg: DexConfig,
  ctx = commandContext(),
  deps?: Parameters<typeof runConchRescueMessage>[0]["deps"],
) {
...
describe("Conch rescue message", () => {
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "conch-rescue-"));
  });
  ...
  it("recognizes the Conch rescue command", () => {
    expect(extractConchRescueMessage("/conch status")).toBe("status");
    expect(extractConchRescueMessage("/conch")).toBe("");
    expect(extractConchRescueMessage("/status")).toBeNull();
  });

  it("denies rescue when sandboxing is active", async () => {
    await expect(
      runRescue("/conch status", {
  ...
  it("refuses TUI handoff from remote rescue", async () => {
    const cfg: DexConfig = { conch: { rescue: { enabled: true } } };
    const deps = {
      runTui: vi.fn(async () => {
    ...
    await expect(
      runRescue("/conch talk to agent", cfg, commandContext(), deps),
    ).resolves.toContain("cannot open the local TUI");
    await expect(runRescue("/conch chat", cfg, commandContext(), deps)).resolves.toContain(
  ...
  it("refuses plugin install from remote rescue", async () => {
    ...
    await expect(
      runRescue("/conch plugin install clawhub:openclaw-demo", cfg, commandContext(), deps),
  ...
  it("allows plugin list and search from remote rescue", async () => {
    ...
    await expect(
      runRescue("/conch plugins list", cfg, commandContext(), deps),
    ).resolves.toContain("plugin rows");
    await expect(
      runRescue("/conch plugins search calendar", cfg, commandContext(), deps),
  ...
  it("queues and applies persistent writes through conversational approval", async () => {
    ...
    const cfg: DexConfig = { conch: { rescue: { enabled: true } } };
    await expect(runRescue("/conch set default model openai/gpt-5.2", cfg)).resolves.toContain(
      "Reply /conch yes to apply",
    );
    await expect(runRescue("/conch yes", cfg)).resolves.toContain(
  ...
  it("queues and applies gateway restart through conversational approval", async () => {
    ...
    await expect(
      runRescue("/conch restart gateway", cfg, commandContext(), deps),
    ).resolves.toBe("Plan: restart the Gateway. Reply /conch yes to apply.");
    await expect(runRescue("/conch yes", cfg, commandContext(), deps)).resolves.toContain(
      "[conch] done: gateway.restart",
    );
  ...
  it("does not queue persistent rescue approval when expiry would exceed the Date range", async () => {
    ...
    try {
      ...
      await expect(
        runRescue("/conch restart gateway", cfg, commandContext()),
      ).resolves.toContain("expiry clock is invalid");

      await expect(fs.readdir(path.join(tempDir, "conch", "rescue-pending"))).rejects.toThrow(
        /ENOENT/,
      );
    } finally {
      vi.useRealTimers();
  ...
  it("rejects pending rescue approvals with invalid persisted expiry", async () => {
    ...
    const pendingDir = path.join(tempDir, "conch", "rescue-pending");
    ...
    await expect(runRescue("/conch yes", cfg, commandContext(), deps)).resolves.toBe(
      "No pending Conch rescue change is waiting for approval.",
    );
  ...
  it("queues and applies agent creation through conversational approval", async () => {
    ...
    await expect(
      runRescue("/conch create agent work workspace /tmp/work", cfg, commandContext(), deps),
    ).resolves.toBe(
      "Plan: create agent work with workspace /tmp/work. Reply /conch yes to apply.",
    );
    await expect(runRescue("/conch yes", cfg, commandContext(), deps)).resolves.toContain(
      "[conch] done: agents.create",
    );
