...
function resolveAgentEntry(cfg: DexConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: DexConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: DexConfig, agentId?: string): boolean {
...
export function resolveConchRescuePolicy(
  input: ConchRescuePolicyInput,
): ConchRescueDecision {
  const rescue = input.cfg.conch?.rescue;
  ...
  if (!enabled) {
    return {
      ...
      reason: "disabled",
      message:
        "Conch rescue is disabled. Set conch.rescue.enabled=true or use YOLO host posture with sandboxing off.",
    };
  }
  if (sandboxActive) {
    return {
      ...
      message:
        "Conch rescue is blocked because OpenClaw sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  if (configuredEnabled === "auto" && !yolo) {
    return {
      ...
      reason: "not-yolo",
      message:
        "Conch rescue auto-mode only opens in YOLO host posture: tools.exec.security=full, tools.exec.ask=off, and sandboxing off.",
    };
  }
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Conch rescue only accepts commands from an OpenClaw owner.",
    };
  }
  if (ownerDmOnly && !input.isDirectMessage) {
    return {
      ...
      message: "Conch rescue is restricted to owner DMs by default.",
    };
  }
  return {
    allowed: true,
