...
import { runConchTui } from "./tui-backend.js";
...
describe("runConchTui", () => {
  it("runs Conch inside the shared TUI shell", async () => {
    let runTuiCalls = 0;
    ...
    expect(options.session).toBe("agent:conch:main");
    expect(options.historyLimit).toBe(200);
    expect(options.config).toEqual({});
    expect(options.title).toBe("dex conch");
    if (!options.backend || typeof options.backend !== "object") {
      throw new Error("expected conch TUI backend");
    }
  });
});
