...
function collectConfigEnvVarsByTarget(cfg?: DexConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: DexConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: DexConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.DEX_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
