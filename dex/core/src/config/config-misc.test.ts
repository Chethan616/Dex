...
import { buildWebSearchProviderConfig, withTempHome, writeDexConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = DexSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  ...
  it("rejects non-string $schema", () => {
    const result = DexSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeDexConfig(home, {
        gateway: { bind: "${DEX_BIND}" },
      });

      const prev = process.env.DEX_BIND;
      process.env.DEX_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.DEX_BIND;
        } else {
          process.env.DEX_BIND = prev;
        }
      }
    });
  });
