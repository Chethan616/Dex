...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.DEX_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.DEX_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.DEX_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `dex doctor --fix` once the
