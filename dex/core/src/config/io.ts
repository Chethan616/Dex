...
import { collectManifestModelIdNormalizationPolicies } from "@dexagent/model-catalog-core/provider-model-id-normalization";
...
import type { DexConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: DexConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: DexConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: DexConfig) => Promise<unknown>;
};
...
function coerceConfig(value: unknown): DexConfig {
...
function stampConfigVersion(cfg: DexConfig, version?: string): DexConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: DexConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        ...
        "Check: `openclaw --version`, `which openclaw`, and `dex gateway status --deep`.",
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.DEX_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.DEX_UPDATE_POST_CORE)
  );
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as DexConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: DexConfig): DexConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: DexConfig): DexConfig {
  ...
  function validateSuspiciousRecoveryBackup(parsed: unknown): boolean {
    try {
      ...
      const loadValidationPluginMetadataSnapshot = (config: DexConfig) => {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): DexConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as DexConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: DexConfig = {};
  ...
  async function readBestEffortConfigLocal(): Promise<DexConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffortLocal(): Promise<DexConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as DexConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as DexConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as DexConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.DEX_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.DEX_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.DEX_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.DEX_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
...
// module scope. `DEX_CONFIG_PATH` (and friends) are expected to work even
...
export function projectConfigOntoRuntimeSourceSnapshot(config: DexConfig): DexConfig {
...
export async function readSourceConfigBestEffort(): Promise<DexConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: DexConfig = nextCfg;
