...
describe("config io write prepare", () => {
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("dex config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('dex config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: DexConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
