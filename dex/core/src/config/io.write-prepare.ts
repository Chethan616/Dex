...
import { normalizeConfiguredProviderCatalogModelId } from "@dexagent/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  dex config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  dex config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: DexConfig,
  pathSegments: string[],
): { changed: boolean; next: DexConfig } {
