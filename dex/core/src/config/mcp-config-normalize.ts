...
type DexMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_DEX_TRANSPORT: Record<string, DexMcpHttpTransport | "stdio"> = {
...
export function resolveOpenClawMcpTransportAlias(
  value: unknown,
): DexMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_DEX_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_DEX_TRANSPORT, normalizeMcpString(value));
}
