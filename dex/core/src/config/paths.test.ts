...
describe("oauth paths", () => {
  it("prefers DEX_OAUTH_DIR over DEX_STATE_DIR", () => {
    const env = {
      DEX_OAUTH_DIR: "/custom/oauth",
      DEX_STATE_DIR: "/custom/state",
  ...
  it("derives oauth path from DEX_STATE_DIR when unset", () => {
    const env = {
      DEX_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ DEX_GATEWAY_PORT: "19001" })),
    ).toBe(19001);
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ DEX_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ DEX_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ DEX_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back to config when env ports exceed TCP bounds", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ DEX_GATEWAY_PORT: "65536" })),
    ).toBe(19003);
    expect(
      resolveGatewayPort(
        { gateway: { port: 19004 } },
        envWith({ DEX_GATEWAY_PORT: "127.0.0.1:65536" }),
      ),
    ).toBe(19004);
    expect(
      resolveGatewayPort(
        { gateway: { port: 19005 } },
        envWith({ DEX_GATEWAY_PORT: "[::1]:65536" }),
      ),
    ).toBe(19005);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ DEX_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ DEX_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ DEX_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  function expectOpenClawHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.DEX_HOME;
    if (!configuredHome) {
      throw new Error("DEX_HOME must be set for this assertion helper");
    }
    const resolvedHome = path.resolve(configuredHome);
    expect(resolveStateDir(env)).toBe(path.join(resolvedHome, ".dex"));

    const candidates = resolveDefaultConfigCandidates(env);
    expect(candidates[0]).toBe(path.join(resolvedHome, ".dex", "openclaw.json"));
  }

  it("uses DEX_STATE_DIR when set", () => {
    const env = {
      DEX_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;

    expect(resolveStateDir(env, () => "/home/test")).toBe(path.resolve("/new/state"));
  });

  it("normalizes relative DEX_STATE_DIR overrides to absolute paths", () => {
    const env = {
      DEX_STATE_DIR: ".",
  ...
  it("prefers DEX_HOME over HOME for default state/config locations", () => {
    const env = {
      DEX_HOME: "/srv/openclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv;
  ...
  it("orders default config candidates in a stable order", () => {
    ...
    const expected = [
      path.join(resolvedHome, ".dex", "openclaw.json"),
      path.join(resolvedHome, ".dex", "clawdbot.json"),
  ...
  it("prefers ~/.dex when it exists and legacy dir is missing", async () => {
  ...
  it("falls back to existing legacy state dir when ~/.dex is missing", async () => {
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    await withTempDir({ prefix: "openclaw-config-" }, async (root) => {
      const legacyDir = path.join(root, ".dex");
      await fs.mkdir(legacyDir, { recursive: true });
...
describe("resolveIncludeRoots", () => {
  ...
  it("returns an empty list when DEX_INCLUDE_ROOTS is unset or blank", () => {
    expect(resolveIncludeRoots(envWith({}), () => HOME)).toStrictEqual([]);
    expect(resolveIncludeRoots(envWith({ DEX_INCLUDE_ROOTS: "" }), () => HOME)).toStrictEqual(
      [],
    );
    expect(
      resolveIncludeRoots(envWith({ DEX_INCLUDE_ROOTS: "   " }), () => HOME),
    ).toStrictEqual([]);
  });
  ...
  it("drops empty entries and preserves de-duplicated order for repeated roots", () => {
    const a = path.resolve(path.parse(process.cwd()).root, "shared", "a");
    const env = envWith({
      DEX_INCLUDE_ROOTS: ["", a, "  ", a].join(path.delimiter),
    });
