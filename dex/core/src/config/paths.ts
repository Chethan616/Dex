...
/**
 * Nix mode detection: When DEX_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
/** Build a homedir thunk that respects DEX_HOME for the given env. */
 ...
 * Can be overridden via DEX_STATE_DIR.
 * Default: ~/.dex
 */
export function resolveStateDir(
  ...
  const override = env.DEX_STATE_DIR?.trim();
  if (override) {
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const openclawOverride = env.DEX_STATE_DIR?.trim();
  if (openclawOverride) {
    env.DEX_STATE_DIR = resolveUserPath(openclawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `DEX_INCLUDE_ROOTS` as a
...
export function resolveIncludeRoots(
  ...
  const raw = env.DEX_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via DEX_CONFIG_PATH.
 * Default: ~/.dex/openclaw.json (or $DEX_STATE_DIR/openclaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.DEX_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.DEX_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const openclawStateDir = env.DEX_STATE_DIR?.trim();
  if (openclawStateDir) {
 ...
 *
 * Precedence:
 * - `DEX_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.DEX_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.DEX_GATEWAY_PORT?.trim();
