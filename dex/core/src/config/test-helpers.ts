...
import { resetConfigRuntimeState, type DexConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        DEX_MPM_CATALOG_PATHS: undefined,
        DEX_LOAD_SHELL_ENV: undefined,
        DEX_DEFER_SHELL_ENV_FALLBACK: undefined,
        DEX_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeDexConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.DEX_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected DEX_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeDexConfig(home, config);
