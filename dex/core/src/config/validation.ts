...
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@dexagent/net-policy/ip";
...
import type { DexConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: DexConfig): DexConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.dex.run/reference/secretref-credential-surface";
...
function collectRawBundledChannelConfigIssues(config: DexConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: DexConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: DexConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: DexConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: DexConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DexSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as DexConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: DexConfig | null | undefined;
  ...
  const ensureCompatConfig = (): DexConfig => {
