...
function hasConfiguredAuthProfiles(cfg: DexConfig): boolean {
  return (
...
type PreparedCronRunContext = {
  ...
  cfgWithAgentDefaults: DexConfig;
  agentId: string;
...
function resolveCronActiveRuntimeConfig(cfg: DexConfig): DexConfig {
...
async function prepareCronRunContext(params: {
  ...
  const cfgWithAgentDefaults: DexConfig = {
    ...runtimeCfg,
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.DEX_TEST_FAST === "1";
