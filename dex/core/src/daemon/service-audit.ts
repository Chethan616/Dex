...
function auditGatewayToken(
  ...
  issues.push({
    ...
    detail: "Run `dex gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `dex gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  ...
  issues.push({
    ...
    message: `Gateway service was installed by Dex ${serviceVersion}; current CLI is ${VERSION}.`,
