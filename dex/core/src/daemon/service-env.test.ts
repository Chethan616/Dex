...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DEX_GATEWAY_PORT).toBe("18789");
    expect(env.DEX_GATEWAY_TOKEN).toBeUndefined();
    expect(env.DEX_SERVICE_MARKER).toBe("openclaw");
  ...
  it("passes through DEX_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.DEX_WRAPPER).toBe("/usr/local/bin/openclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".dex", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DEX_PROFILE: "work" },
      port: 18789,
    });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        DEX_PROFILE: "work",
        DEX_SYSTEMD_UNIT: "openclaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.DEX_SYSTEMD_UNIT).toBe("openclaw-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        DEX_SYSTEMD_UNIT: "openclaw-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", DEX_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        DEX_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.DEX_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the OpenClaw-owned launchd marker for macOS node services", () => {
    ...
    expect(env.DEX_LAUNCHD_LABEL).toBe("ai.openclaw.node");
  });

  it("passes through DEX_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DEX_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.DEX_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through DEX_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DEX_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.DEX_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits DEX_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        DEX_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".dex"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", DEX_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", DEX_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".dex"));
  });

  it("uses DEX_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", DEX_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in DEX_STATE_DIR", () => {
    const env = { HOME: "/Users/test", DEX_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { DEX_STATE_DIR: "C:\\State\\openclaw" };
