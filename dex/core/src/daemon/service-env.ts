...
function readServiceProxyEnvironment(
  ...
  const proxyUrl = normalizeOptionalString(env.DEX_PROXY_URL);
  return proxyUrl ? { DEX_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.DEX_PROFILE;
  const wrapperPath = normalizeOptionalString(env.DEX_WRAPPER);
  ...
  return {
    ...
    DEX_GATEWAY_PORT: String(port),
    DEX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    DEX_SYSTEMD_UNIT: systemdUnit,
    DEX_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    DEX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    DEX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    DEX_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.DEX_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.DEX_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    DEX_GATEWAY_TOKEN: gatewayToken,
    DEX_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    DEX_STATE_DIR: sharedEnv.stateDir,
    DEX_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.DEX_STATE_DIR;
