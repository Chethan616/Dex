...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<DexConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<DexConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.dex.run/tools/web";
...
function rawKeyValue(config: DexConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: DexConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: DexConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: DexConfig, result: DexConfig): DexConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as DexConfig["plugins"],
  };
}
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        `Docs: ${entry.docsUrl ?? "https://docs.dex.run/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
