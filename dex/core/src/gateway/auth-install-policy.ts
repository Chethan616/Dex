...
type GatewayInstallAuthMode = NonNullable<NonNullable<DexConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: DexConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.DEX_GATEWAY_PASSWORD) ||
