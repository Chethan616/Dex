...
describe("runBootOnce", () => {
  ...
  it("wraps BOOT.md content in internal-runtime-context delimiters so verbatim echoes get stripped", async () => {
    ...
    expect(message).toContain("<<<BEGIN_DEX_INTERNAL_CONTEXT>>>");
    expect(message).toContain("<<<END_DEX_INTERNAL_CONTEXT>>>");
  ...
  it("escapes literal internal-runtime-context delimiters in user-supplied BOOT.md to prevent confusion with the wrapper", async () => {
    ...
    expect((message.match(/<<<BEGIN_DEX_INTERNAL_CONTEXT>>>/g) ?? []).length).toBe(1);
    expect((message.match(/<<<END_DEX_INTERNAL_CONTEXT>>>/g) ?? []).length).toBe(1);
    expect(message).toContain("[[DEX_INTERNAL_CONTEXT_BEGIN]]");
    expect(message).toContain("[[DEX_INTERNAL_CONTEXT_END]]");
  });
