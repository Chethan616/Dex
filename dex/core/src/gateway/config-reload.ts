...
function asPluginInstallConfig(records: PluginInstallRecords): DexConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: DexConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: DexConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: DexConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
