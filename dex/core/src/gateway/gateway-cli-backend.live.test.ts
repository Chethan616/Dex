...
import { clearRuntimeConfigSnapshot, type DexConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.DEX_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.DEX_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.DEX_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.DEX_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.DEX_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "DEX_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<DexConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "DEX_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.DEX_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.DEX_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      ...
      const cliCommand = process.env.DEX_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `DEX_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "DEX_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.DEX_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.DEX_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.DEX_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "DEX_LIVE_CLI_BACKEND_IMAGE_MODE requires DEX_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }
      ...
      const disableMcpConfig = process.env.DEX_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfg: DexConfig = {};
      const cfgWithCliBackends = cfg as DexConfig & {
        agents?: {
          defaults?: {
