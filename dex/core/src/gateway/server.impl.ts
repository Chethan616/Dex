...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.DEX_GATEWAY_STARTUP_TRACE);
  let timelineConfig: DexConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.DEX_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`dex config set gateway.auth.mode token\` and \`dex config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.DEX_TEST_MINIMAL_GATEWAY === "1";
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: DexConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: DexConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
