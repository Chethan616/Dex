...
function createSingleAgentAvatarConfig(workspace: string): DexConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("session list search includes direct-session origin display labels", () => {
    const cfg = { agents: { list: [{ id: "main", default: true }] } } as DexConfig;
    const store = {
...
describe("resolveSessionModelIdentityRef", () => {
  const resolveLegacyIdentityRef = (cfg: DexConfig, modelProvider?: string) =>
  ...
  test("infers provider from configured provider catalogs when allowlist is absent", () => {
    ...
    } as unknown as DexConfig["models"];
