...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.DEX_CONFIG_PATH) {
    configPaths.add(process.env.DEX_CONFIG_PATH);
  }
  if (process.env.DEX_STATE_DIR) {
    configPaths.add(path.join(process.env.DEX_STATE_DIR, "openclaw.json"));
  }
...
function applyGatewaySkipEnv() {
  ...
  process.env.DEX_BUNDLED_PLUGINS_DIR = tempHome
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["DEX_GATEWAY_TOKEN"]);
  const prev = process.env.DEX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.DEX_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.DEX_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
