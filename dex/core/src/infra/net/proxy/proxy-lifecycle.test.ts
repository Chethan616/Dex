...
describe("startProxy", () => {
  ...
  it("uses DEX_PROXY_URL when config proxyUrl is omitted", async () => {
    process.env["DEX_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("prefers config proxyUrl over DEX_PROXY_URL", async () => {
    process.env["DEX_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("uses HTTPS proxy URLs from DEX_PROXY_URL", async () => {
    process.env["DEX_PROXY_URL"] = "https://127.0.0.1:3128";
  ...
  it("passes configured proxy CA trust to Proxyline", async () => {
    ...
    expect(process.env["DEX_PROXY_CA_FILE"]).toBe(caFile);
    expect(installGlobalProxyMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("loads inherited HTTPS proxy CA trust for child routing", () => {
    ...
    process.env["DEX_PROXY_ACTIVE"] = "1";
    process.env["DEX_PROXY_LOOPBACK_MODE"] = "gateway-only";
    process.env["HTTP_PROXY"] = "https://proxy.example:8443";
    process.env["DEX_PROXY_CA_FILE"] = caFile;
  ...
  it("sets process proxy env vars for inherited clients", async () => {
    ...
    expect(process.env["HTTP_PROXY"]).toBe("http://127.0.0.1:3128");
    expect(process.env["HTTPS_PROXY"]).toBe("http://127.0.0.1:3128");
    expect(process.env["DEX_PROXY_ACTIVE"]).toBe("1");
    expect(process.env["DEX_PROXY_LOOPBACK_MODE"]).toBe("gateway-only");
  });
  ...
  it("persists loopbackMode in env for forked child CLIs", async () => {
    ...
    expect(process.env["DEX_PROXY_LOOPBACK_MODE"]).toBe("block");
    expect(getActiveManagedProxyLoopbackMode()).toBe("block");

    await stopProxy(handle);
    process.env["DEX_PROXY_ACTIVE"] = "1";
    process.env["DEX_PROXY_LOOPBACK_MODE"] = "proxy";
  ...
  it("restores previous proxy env and stops Proxyline on stop", async () => {
    ...
    expect(process.env["DEX_PROXY_ACTIVE"]).toBeUndefined();
    expect(proxylineStopMock).toHaveBeenCalledOnce();
    expect(forceResetGlobalDispatcherMock).toHaveBeenCalledTimes(2);
  });
