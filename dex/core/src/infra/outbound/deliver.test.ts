...
const matrixChunkConfig: DexConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as DexConfig["channels"],
};
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: DexConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as DexConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as DexConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as DexConfig;
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "matrix",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as DexConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as DexConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".dex", "workspace-work")),
      ),
    ).toBe(true);
  ...
  it("includes OpenClaw tmp root in plugin mediaLocalRoots", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as DexConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as DexConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
