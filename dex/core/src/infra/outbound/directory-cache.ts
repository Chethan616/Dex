...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DexConfig | null = null;
   ...
   */
  get(key: string, cfg: DexConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: DexConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: DexConfig): void {
