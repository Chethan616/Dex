...
const MATRIX_REPO_INSTALL_COMMAND = `dex plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    ...
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "dex plugins install @openclaw/matrix",
      ...
      'Reinstall with "dex plugins install @openclaw/matrix".',
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<dex plugins install @openclaw/matrix>" when you want to return to the standard Matrix plugin.',
