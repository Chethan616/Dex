...
import { asNullableObjectRecord as readRecord } from "@dexagent/normalization-core/record-coerce";
import {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OpenClaw. You can keep running the web gateway without it: \`${formatCliCommand("dex gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
