...
export const DEX_MAIN_PACKAGE_SPEC = "github:openclaw/openclaw#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_DEX_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.DEX_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.DEX_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return DEX_MAIN_PACKAGE_SPEC;
  }
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmOpenClawSourceInstallSpec(spec) ? [PNPM_DEX_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
