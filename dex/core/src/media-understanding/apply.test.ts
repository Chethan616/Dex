...
function createGroqAudioConfig(): DexConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DexConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): DexConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: DexConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: DexConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
