...
import type {
  DexProviderIndex,
  DexProviderIndexPluginInstall,
  DexProviderIndexPlugin,
  DexProviderIndexProviderAuthChoice,
  DexProviderIndexProvider,
} from "./types.js";

const DEX_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): DexProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizePlugin(value: unknown): DexProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): DexProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): DexProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): DexProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly DexProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is DexProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): DexProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeDexProviderIndex(value: unknown): DexProviderIndex | undefined {
  if (!isRecord(value) || value.version !== DEX_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, DexProviderIndexProvider> = {};
  ...
  return {
    version: DEX_PROVIDER_INDEX_VERSION,
