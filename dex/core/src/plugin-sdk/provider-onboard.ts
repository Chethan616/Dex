...
import { findNormalizedProviderKey } from "@dexagent/model-catalog-core/provider-id";
...
export type { DexConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: DexConfig, ...args: TArgs) => DexConfig;
  applyConfig: (cfg: DexConfig, ...args: TArgs) => DexConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: DexConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: DexConfig, preset: TParams) => DexConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: DexConfig): {
  next: DexConfig;
  changed: boolean;
} {
