...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openclawHome: process.env.DEX_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("DEX_HOME", snapshot.openclawHome);
  restoreKey("DEX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked DEX_HOME.
  delete process.env.DEX_HOME;
  process.env.DEX_STATE_DIR = path.join(base, ".dex");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".dex", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
