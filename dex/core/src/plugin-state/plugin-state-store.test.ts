...
let testState: DexTestState | undefined;

beforeAll(async () => {
  testState = await createDexTestState({ label: "plugin-state-store" });
...
describe("plugin state keyed store", () => {
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createDexTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openDexStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
