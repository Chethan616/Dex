...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as DexConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as DexConfig;
    const load = vi.fn(async (config?: DexConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: DexConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
