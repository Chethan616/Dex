...
export type ConfigScopedRuntimeCache<T> = WeakMap<DexConfig, Map<string, T>>;

export type ConfigScopedPromiseLoader<T> = {
  load(config?: DexConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: DexConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<DexConfig, Promise<T>>();

  const createPromise = (config?: DexConfig): Promise<T> => {
  ...
  return {
    async load(config?: DexConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<DexConfig, Promise<T>>();
    },
  };
}
