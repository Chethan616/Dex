...
import type {
  DexPluginHookOptions,
  DexPluginToolFactory,
  DexPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type DexPluginConfigSchema = {
...
export type ProviderPlugin = {
  ...
  /**
   * Legacy auth-profile ids that should be retired by `dex doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `dex doctor` should offer.
   *
...
export type DexPluginGatewayMethod = {
  method: string;
...
export type DexPluginCommandDefinition = {
...
export type DexPluginHttpRouteAuth = "gateway" | "plugin";
export type DexPluginHttpRouteMatch = "exact" | "prefix";
export type DexPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type DexPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type DexPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type DexPluginHttpRouteParams = {
  path: string;
  handler: DexPluginHttpRouteHandler;
  handleUpgrade?: DexPluginHttpRouteUpgradeHandler;
  auth: DexPluginHttpRouteAuth;
  match?: DexPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: DexPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type DexPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type DexPluginCliContext = {
  /**
...
export type DexPluginCliRegistrar = (ctx: DexPluginCliContext) => void | Promise<void>;
 ...
 */
export type DexPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type DexPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `dex nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `dex nodes`, so a descriptor named
   * `"camera"` exposes `dex nodes camera`.
   */
  descriptors?: DexPluginCliCommandDescriptor[];
};

export type DexPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type DexPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type DexPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type DexPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type DexPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: DexPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type DexPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: DexPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<DexPluginNodeInvokeTransportResult>;
};

export type DexPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type DexPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: DexPluginNodeInvokePolicyContext,
  ) => Promise<DexPluginNodeInvokePolicyResult> | DexPluginNodeInvokePolicyResult;
};

export type DexPluginSecurityAuditContext = {
  config: DexConfig;
...
export type DexPluginSecurityAuditCollector = (
  ctx: DexPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type DexGatewayDiscoveryAdvertiseContext = {
...
export type DexGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: DexGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type DexPluginServiceContext = {
  config: DexConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type DexPluginService = {
  id: string;
  start: (ctx: DexPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: DexPluginServiceContext) => void | Promise<void>;
};

export type DexPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type DexPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: DexPluginReloadRegistration;
  nodeHostCommands?: DexPluginNodeHostCommand[];
  securityAuditCollectors?: DexPluginSecurityAuditCollector[];
  register?: (api: DexPluginApi) => void;
  activate?: (api: DexPluginApi) => void;
};

export type DexPluginModule = DexPluginDefinition | ((api: DexPluginApi) => void);
...
export type PluginConfigMigration = (config: DexConfig) =>
  | {
      config: DexConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `dex migrate`. */
...
export type DexPluginSessionStateApi = {
...
export type DexPluginSessionWorkflowApi = {
...
export type DexPluginSessionControlsApi = {
...
export type DexPluginSessionApi = {
  state: DexPluginSessionStateApi;
  workflow: DexPluginSessionWorkflowApi;
  controls: DexPluginSessionControlsApi;
};

export type DexPluginAgentEventsApi = {
...
export type DexPluginAgentApi = {
  events: DexPluginAgentEventsApi;
};

export type DexPluginRunContextApi = {
...
export type DexPluginLifecycleApi = {
...
export type DexPluginApi = {
   ...
   */
  session: DexPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: DexPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: DexPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: DexPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | DexPluginToolFactory,
    opts?: DexPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: DexPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: DexPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: DexPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: DexPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: DexPluginCliRegistrar,
    opts?: {
  ...
  /**
   * Register a plugin-owned node feature command group under `dex nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: DexPluginCliRegistrar,
    opts?: DexPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: DexPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: DexPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: DexPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: DexPluginSecurityAuditCollector) => void;
  registerService: (service: DexPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: DexGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `dex migrate` (migration capability). */
