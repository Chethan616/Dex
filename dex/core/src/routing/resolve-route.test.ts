...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: DexConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: DexConfig };
    expected: ResolvedRouteExpectation;
  }) => {
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<DexConfig["bindings"]>[number];
