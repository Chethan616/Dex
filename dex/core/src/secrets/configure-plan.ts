...
export function buildConfigureCandidates(config: DexConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: DexConfig;
  authoredDexConfig?: DexConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredDexConfig ?? params.config;
