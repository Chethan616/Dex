...
function getSecretProviders(config: DexConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: DexConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: DexConfig): Array<{ value: SecretRefSource; label: string }> {
...
function resolveConfigureAgentId(config: DexConfig, explicitAgentId?: string): string {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredDexConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
