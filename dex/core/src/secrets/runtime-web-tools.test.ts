...
function setConfiguredFetchProviderKey(configTarget: DexConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: DexConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: DexConfig, provider: ProviderUnderTest): unknown {
