...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    restoreEnv("DEX_HOME", previousOpenClawHome);
    restoreEnv("DEX_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".dex", "settings", "tts.json");
  ...
  it("derives the default prefs path from DEX_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      ...
      vi.stubEnv("DEX_CONFIG_PATH", path.join(stateDir, "openclaw.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
