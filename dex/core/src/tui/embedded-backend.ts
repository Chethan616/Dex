...
function hasProviderWildcardModelAllowlist(cfg: DexConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: DexConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: DexConfig) {
  return cfg.models?.mode === "replace" && hasProviderWildcardModelAllowlist(cfg);
}

async function loadEmbeddedTuiModelCatalog(cfg: DexConfig) {
