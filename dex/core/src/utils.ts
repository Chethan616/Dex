...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".dex");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.DEX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$DEX_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$DEX_HOME`. */
...
// Configuration root; can be overridden via DEX_STATE_DIR.
