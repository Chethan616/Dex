...
export const zh_CN = {
  ...
  wizard: {
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `dex doctor`。",
      previewTitle: "迁移预览",
      source: "迁移来源",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 DEX_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    skills: {
      ...
      docsLine: "文档：https://docs.dex.run/skills",
      enterEnv: "输入 {env}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.dex.run/web/control-ui",
      ...
      gatewayTokenStored:
        "存储位置：$DEX_CONFIG_PATH（默认 ~/.dex/openclaw.json）中的 gateway.auth.token，或 DEX_GATEWAY_TOKEN。",
      ...
      restart: "重启",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.dex.run/security",
      ...
      webDocs: "文档：https://docs.dex.run/tools/web",
      ...
      workspaceDocs: "文档：https://docs.dex.run/concepts/agent-workspace",
