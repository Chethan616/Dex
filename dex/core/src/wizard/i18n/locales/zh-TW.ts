...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 Dex 設定",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 DEX_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密碼存入 Dex 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 Dex 設定",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.dex.run/skills",
      enterEnv: "輸入 {env}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.dex.run/web/control-ui",
      ...
      gatewayTokenStored:
        "儲存位置：$DEX_CONFIG_PATH（預設 ~/.dex/openclaw.json）中的 gateway.auth.token，或 DEX_GATEWAY_TOKEN。",
      ...
      restart: "重新啟動",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.dex.run/security",
      ...
      webDocs: "文件：https://docs.dex.run/tools/web",
      ...
      workspaceDocs: "文件：https://docs.dex.run/concepts/agent-workspace",
