...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers DEX_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.DEX_GATEWAY_TOKEN;
    process.env.DEX_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps DEX_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.DEX_GATEWAY_TOKEN;
    process.env.DEX_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.DEX_GATEWAY_PASSWORD;
    process.env.DEX_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "DEX_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
  ...
  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.DEX_GATEWAY_TOKEN = previous;
      }
    }
  });
