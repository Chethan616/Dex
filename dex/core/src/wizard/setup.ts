...
async function writeWizardConfigFile(
  configInput: DexConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: DexConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: DexConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: DexConfig | undefined = baseConfig;
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("dex doctor")}`,
        `Inspect: ${formatCliCommand("dex plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("dex configure"),
  });
  ...
  let localGatewayToken = process.env.DEX_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.DEX_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: DexConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
