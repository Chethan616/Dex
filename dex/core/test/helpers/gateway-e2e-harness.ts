...
import { createDexTestInstance, type DexTestInstance } from "./openclaw-test-instance.js";
...
export type GatewayInstance = DexTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createDexTestInstance({ name });
  try {
