...
describe("scripts/run-vitest", () => {
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "60000",
      DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e configs", () => {
    ...
    expect(
      ...
      PATH: "/usr/bin",
      DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "60000",
      DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
    });
    expect(
      resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, [
        "run",
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  fs.writeFileSync(process.env.DEX_DELEGATED_SIGNAL_CHILD_PID!, String(process.pid));",
        "  await new Promise(() => {});",
        "});",
        "",
      ].join("\n"),
    ...
    const runner = spawn(
      ...
      ["scripts/run-vitest.mjs", fixturePath, "--reporter=verbose"],
      {
        env: {
          ...process.env,
          DEX_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
