...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.DEX_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.DEX_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.DEX_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    DEX_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    DEX_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("skips the lock when a parent changed gate already holds it", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          DEX_TEST_HEAVY_CHECK_LOCK_HELD: "1",
        },
      ),
    ).toBe(false);
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          DEX_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  beforeAll(async () => {
    ...
    const previous = process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.DEX_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  });
  ...
  it("rejects malformed parallel full-suite overrides", () => {
    ...
    ).toThrow("DEX_TEST_PROJECTS_PARALLEL must be a positive integer; got: 3x");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DEX_TEST_PROJECTS_PARALLEL: "0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DEX_TEST_PROJECTS_PARALLEL must be a positive integer; got: 0");
  });
  ...
  it("rejects malformed conservative worker budget values", () => {
    ...
    ).toThrow("DEX_VITEST_MAX_WORKERS must be a positive integer; got: 1e0");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DEX_TEST_WORKERS: "1 worker",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DEX_TEST_WORKERS must be a positive integer; got: 1 worker");
  });

  it("keeps serial untargeted runs on aggregate shards", () => {
    const previousParallel = process.env.DEX_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.DEX_TEST_PROJECTS_SERIAL;
    delete process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    ...
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.DEX_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.DEX_TEST_PROJECTS_SERIAL;
      } else {
        process.env.DEX_TEST_PROJECTS_SERIAL = previousSerial;
      }
    }
  });
  ...
  it("expands untargeted local runs to leaf project configs by default", () => {
    ...
    const previousVitestMaxWorkers = process.env.DEX_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.DEX_TEST_WORKERS;
    delete process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.DEX_TEST_PROJECTS_PARALLEL;
    delete process.env.DEX_TEST_PROJECTS_SERIAL;
    ...
    try {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.DEX_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.DEX_TEST_WORKERS;
      } else {
        process.env.DEX_TEST_WORKERS = previousTestWorkers;
      }
    }
  });

  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.DEX_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.DEX_TEST_PROJECTS_SERIAL;
    delete process.env.DEX_TEST_PROJECTS_PARALLEL;
    process.env.DEX_TEST_PROJECTS_SERIAL = "1";
    process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.DEX_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.DEX_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.DEX_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.DEX_TEST_PROJECTS_PARALLEL;
    delete process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    process.env.DEX_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
  ...
  it("rejects malformed full-suite expansion parallel overrides", () => {
    ...
    const previousParallel = process.env.DEX_TEST_PROJECTS_PARALLEL;
    delete process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
    process.env.DEX_TEST_PROJECTS_PARALLEL = "6x";
    try {
      expect(() => buildFullSuiteVitestRunPlans([], process.cwd())).toThrow(
        "DEX_TEST_PROJECTS_PARALLEL must be a positive integer; got: 6x",
      );
    } finally {
      if (previousLeafShards === undefined) {
        delete process.env.DEX_TEST_PROJECTS_LEAF_SHARDS;
...
describe("scripts/test-projects parallel cache paths", () => {
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { DEX_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.DEX_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds default no-output watchdog settings to non-watch specs", () => {
    ...
    expect(spec?.env.DEX_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
    expect(spec?.env.DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_HEARTBEAT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: {
            DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "25000",
            DEX_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
            PATH: "/usr/bin",
          },
          includeFilePath: null,
          includePatterns: null,
    ...
    expect(specs[0]?.env.DEX_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[0]?.env.DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBeUndefined();
    expect(specs[1]?.env.DEX_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
    expect(specs[1]?.env.DEX_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe("25000");
  });
  ...
  it("allows changed checks to disable automatic silent-run retries", () => {
    ...
    expect(shouldRetryVitestNoOutputTimeout({ DEX_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ DEX_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ DEX_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.DEX_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
