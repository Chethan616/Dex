...
describe("website installer sync workflow", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain('DEX_NO_ONBOARD: "1"');
    expect(workflow).toContain('DEX_NO_PROMPT: "1"');
  ...
  it("syncs verified scripts to openclaw.ai only after all installer checks pass", () => {
    ...
    expect(workflow).toContain("DEX_GH_TOKEN: ${{ secrets.DEX_GH_TOKEN }}");
    expect(workflow).toContain("DEX_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.DEX_GH_TOKEN }}");
