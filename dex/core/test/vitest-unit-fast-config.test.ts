...
describe("unit-fast vitest lane", () => {
  ...
  it("runs cache-friendly tests without the reset-heavy runner or runtime setup", () => {
    ...
    expect(testConfig.include).toContain("src/conch/rescue-policy.test.ts");
    expect(testConfig.include).toContain("src/conch/assistant.configured.test.ts");
  ...
  it("keeps obvious stateful files out of the unit-fast lane", () => {
    ...
    expect(isUnitFastTestFile("src/conch/assistant.test.ts")).toBe(false);
