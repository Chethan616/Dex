...
describe("unit-fast vitest lane", () => {
  ...
  it("runs cache-friendly tests without the reset-heavy runner or runtime setup", () => {
    ...
    expect(testConfig.include).toContain("src/atlas/rescue-policy.test.ts");
    expect(testConfig.include).toContain("src/atlas/assistant.configured.test.ts");
  ...
  it("keeps obvious stateful files out of the unit-fast lane", () => {
    ...
    expect(isUnitFastTestFile("src/atlas/assistant.test.ts")).toBe(false);
