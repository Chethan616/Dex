...
import "../app.ts";
import type { DexApp } from "../app.ts";
...
const mountedApps = new Set<DexApp>();

function collectMountedApps() {
  return new Set<DexApp>([
    ...mountedApps,
    ...document.querySelectorAll<DexApp>("openclaw-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<DexApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<DexApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("openclaw-app") as DexApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__DEX_CONTROL_UI_BASE_PATH__"] = undefined;
