...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.DEX_CONTROL_UI_BUILD_ID?.trim() || process.env.DEX_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__DEX_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(): UserConfig {
  const envBase = process.env.DEX_CONTROL_UI_BASE_PATH?.trim();
  const base = envBase ? normalizeBase(envBase) : "./";
  const controlUiBuildId = resolveControlUiBuildId();
  return {
    base,
    define: {
      DEX_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
