...
// Where Dex is listening.
//
// FORK CHANGE. Upstream dialled ws://localhost:5555 and then scanned a list of
// candidate ports looking for an MCP bridge process. Dex has no bridge
// process: its browser agent hosts this socket itself, on the port it already
// serves HTTP on, so there is exactly one address and nothing to discover.
//
// Scanning also had a cost worth avoiding — it fired requests at half a dozen
// localhost ports belonging to whatever else the owner happens to be running.
let MCP_SERVER_URL = 'ws://127.0.0.1:8766/extension';
let lastKnownPorts = { websocket: 8766, http: 8766 };
...
class ConnectionManager {
  ...
  async discoverServerPorts() {
    // FORK CHANGE: there is nothing to discover. Dex serves this socket from
    // its browser agent on a fixed port, so the scan below would only knock on
    // other people's localhost servers. Kept as a function because the
    // reconnect path calls it; it now just reports the one address.
    return { websocket: 8766, http: 8766, websocketUrl: MCP_SERVER_URL };
