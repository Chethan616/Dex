...
class Supervisor extends ChangeNotifier {
  ...
  Future<bool> _runPreflight() async {
    ...
    if (!located.hasEnvFile) {
      // Same recovery RUN.bat performs: a fresh clone has no .env because it is
      // gitignored, and copying the example gets Dex to a running state. The
      // model key is the one thing it cannot invent, and Settings asks for it.
      try {
        if (located.envExampleFile.existsSync()) {
          located.envExampleFile.copySync(located.envFile.path);
        } else {
          located.envFile.writeAsStringSync('');
        }
      } catch (e) {
        target.finish(BootStatus.failed, detail: 'Could not create .env: $e');
        notifyListeners();
        return false;
      }
    }
